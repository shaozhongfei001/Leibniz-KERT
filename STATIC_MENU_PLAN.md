...
> 目标：在 DSH 左侧栏提供**固定的、刷新后依然存在**的「KERT 平台」菜单入口与面板页，
> 取代当前"刷新后丢失"的动态插件入口。本计划不改动 KERT 业务代码（`kert/`），
...
| 执行 kert CLI | shell/subprocess 未直接暴露为 Remote；需在插件的 host 半区注册一个 `@Remote` 服务（如 `ctx.kert.run`），client 经 ctx 调用（参照 message-feedback 的 Remote 模式） |

## 2. 目标形态

- **左侧栏底部（设置旁）**：静态「KERT 平台」按钮（窄条模式显示「DK」小方块）；
- **完整面板页**：注册为 `settings.section` 的「KERT 平台」页（设置菜单中可见），或点击按钮弹出的 overlay（静态化当前浮层）；
- 点击任一路径打开面板：工作区概览 / 检索 / 数据查询 / 规则评估 / 图谱 / 溯源 / CLI；
- **刷新后所有入口与功能均持久存在**。

## 3. 技术方案

### 3.1 新增包 `packages/client/ui-kert`

```
packages/client/ui-kert/
├── package.json          # name=@deepseek-ai/dsh-client-ui-kert
│                         # dsh.client: { inject: [...], platform: "web" }
│                         # exports: { "./client": .../client.js, ".": .../index.js }
├── tsdown.client.ts      # client bundle 构建配置（参照 ui-commands）
├── src/
│   ├── index.ts          # host 半区：注册 Remote 服务 ctx.kert（执行 kert CLI）
│   ├── client/
│   │   ├── index.ts      # client 半区：apply(ctx) 注册
│   │   │                 #   sidebar.footer.action（静态按钮）
│   │   │                 #   settings.section（KERT 平台页）或 shell.overlay（浮层）
│   │   ├── KertPanel.tsx # 面板组件（移植当前动态插件 UI）
│   │   └── locales.ts    # 中文文案（参照 ui-sidebar 模式）
```

### 3.2 Host 半区：Remote 服务（执行 kert CLI）

- 在 `src/index.ts` 用 `@Remote` 注册服务方法：
  - `kert.inspect()` → `kert inspect --output json`
  - `kert.search/queryData/evaluateRule/graph/trace/run(...)` → 对应 CLI（同当前动态插件 Host 逻辑）
- 执行方式：Host 半区 `ctx.shell`（bash）运行 `/…/kert/.venv/bin/kert <args>`，`PYTHONPATH=…/kert/src`，cwd=`demo_workspace`；
- 安全：仅允许白名单子命令，输出经 JSON 解析返回；日志脱敏。

### 3.3 构建与部署

1. `packages/client/ui-kert` 纳入 monorepo（workspace 依赖声明）；
...
4. 验证：刷新 `http://127.0.0.1:3080` → 侧栏「KERT 平台」按钮存在且功能可用；再次刷新依然存在。

## 4. 实施步骤（约 6-10 步，估计 2-4 小时）

| # | 步骤 | 输出/验证 |
|---|---|---|
| 1 | 脚手架：创建 `ui-kert` 包（package.json/tsdown/exports，参照 ui-commands） | 包可被 pnpm 解析 |
| 2 | host 半区：Remote 服务 `ctx.kert`（bash 执行 kert CLI，白名单） | `@Remote` 方法签名正确 |
...
| **回滚**：删除 `ui-kert` 包 + 重建 + 重启即可完全恢复；不动其他包 | — | 全程只新增一个包，不改既有文件 |
...
2. 面板形态偏好：**A.** 设置菜单里的「KERT 平台」页；**B.** 点击侧栏按钮弹出浮层（推荐，贴近现状）；**C.** 两者都要；
3. 侧栏按钮文字：展开「KERT 平台」/ 窄条「DK」（可调）；
4. 完成后是否**停用**当前动态插件 `kert-1`（避免双入口）。
