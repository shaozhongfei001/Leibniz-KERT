"""KERT CLI（规格 §12）。P0：init / inspect / validate / recover。"""
...
def _exit_code_for(exc: KERTException) -> int:
...
def init_cmd(
    ...
    owner: str = typer.Option("svc_kert", "--owner", help="初始化主体"),
...
def provision_cmd(
    ...
    """供给控制面元数据（知识地图 / 路由策略 / 本体引用 / 知识源能力声明）到工作区（M7.3 第五步）。

    先全量校验源，再逐份原子写入；源内任一份定义**非法**则**一份都不写**。
    **缺失**按"不供给、不报错"处理（与本体引用同口径）：`knowledge_sources.json` 等
    声明缺失不会让部署起不来，但新的运行时消费方（如 M7.1 第二片接线后）会因缺声明而拒绝。
...
def gate_report_cmd(
    workspace_path: Path = typer.Option(..., "--workspace", "-w"),
    reviewer: str = typer.Option("kert_qa", "--reviewer"),
    output: str = typer.Option("text", "--output", help=_COMMON),
):
...
def serve_cmd(
    ...
    """启动 KERT HTTP 服务（含 DSH Web 界面）。"""
    import uvicorn

    from ..api.server import create_app

    app = create_app(Path(workspace_path), service_id=service)
    typer.echo(f"KERT 服务启动: http://{host}:{port}")
