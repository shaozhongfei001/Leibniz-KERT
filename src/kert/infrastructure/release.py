...
2. **缺 git commit 锚点**：治理文档
   ``KERT_STATUS_BASELINE_CANDIDATE.yaml`` 明确登记
   ``kert_git_commit_anchor: null``，evidence manifest 亦将其列为 missing。
...
class ReleaseManifest:
    ...
    def as_dict(self) -> dict:
        """转为可序列化字典。"""
        return {
            "schema": "kert_release_manifest/v1",
...
def build_release_manifest(repo: Path, *, release_id: str | None = None
    ...
    manifest = ReleaseManifest(
        ...
        environment={
            ...
            "profile": os.environ.get("KERT_PROFILE", "dev"),
        },
    )
