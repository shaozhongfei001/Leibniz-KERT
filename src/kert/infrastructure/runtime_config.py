...
职责：集中承载 KERT Python Core 的运行加固配置——profile、API Key 认证、
...
class ObservabilityConfig:
    ...
    log_level: str = "INFO"
    service_name: str = "kert-python-core"
...
def _keys_from_env(env: dict[str, str]) -> tuple[ApiKeyRecord, ...]:
    """从 ``KERT_API_KEYS``/``KERT_API_KEY`` 读取密钥声明。"""
    raw = env.get("KERT_API_KEYS") or env.get("KERT_API_KEY") or ""
...
def _load_config_file(env: dict[str, str]) -> dict:
    ...
    path = env.get("KERT_CONFIG_FILE")
    if not path:
        return {}
    p = Path(path)
    if not p.is_file():
        raise ConfigError(f"KERT_CONFIG_FILE 指向的文件不存在：{path}")
    try:
...
def load_runtime_config(env: dict[str, str] | None = None,
    ...
    profile = (env.get("KERT_PROFILE") or file_cfg.get("profile") or PROFILE_DEV).strip().lower()
    if profile not in VALID_PROFILES:
        raise ConfigError(f"KERT_PROFILE 仅支持 {VALID_PROFILES}，实际为 {profile!r}")
    bind_host = env.get("KERT_BIND_HOST") or file_cfg.get("bind_host") or "127.0.0.1"

    keys = _keys_from_env(env) or _keys_from_file(file_auth)
    auth_enabled = _env_bool(env, "KERT_AUTH_ENABLED", None)
    if auth_enabled is None:
        auth_enabled = bool(file_auth.get("enabled", bool(keys)))
    auth = AuthConfig(
        enabled=bool(auth_enabled),
        header_name=(env.get("KERT_AUTH_HEADER") or file_auth.get("header_name")
                     or "X-API-Key"),
        keys=keys,
    ...
    observability = ObservabilityConfig(
        ...
        log_level=(env.get("KERT_LOG_LEVEL")
                   or file_obs.get("log_level", "INFO")).strip().upper(),
        service_name=(env.get("KERT_SERVICE_NAME")
                      or file_obs.get("service_name", "kert-python-core")).strip(),
        metrics_enabled=bool(obs_metrics),
        metrics_require_admin=bool(obs_metrics_admin),
        tracing_enabled=bool(obs_tracing),
        trace_sample_ratio=_env_float(env, "KERT_TRACE_SAMPLE_RATIO",
...
def validate_runtime_config(cfg: RuntimeConfig) -> list[str]:
    ...
    if not cfg.auth.enabled:
        problems.append("生产 profile 必须启用 API Key 认证（KERT_AUTH_ENABLED=true）")
    if not cfg.rate_limit.enabled:
        problems.append("生产 profile 必须启用限流（KERT_RATE_LIMIT_ENABLED=true）")
