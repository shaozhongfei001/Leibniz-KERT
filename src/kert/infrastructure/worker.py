...
def build_worker_config_from_env(env: dict[str, str] | None = None) -> WorkerConfig:
    ...
    支持的变量：``KERT_WORKER_ID`` / ``KERT_WORKER_JOB_TYPES`` /
    ...
    types_raw = src.get("KERT_WORKER_JOB_TYPES", "").strip()
    job_types = tuple(t.strip() for t in types_raw.split(",") if t.strip())
    worker_id = src.get("KERT_WORKER_ID", "").strip() or default_worker_id()
    return WorkerConfig(
        worker_id=worker_id,
        job_types=job_types,
        lease_seconds=num("KERT_WORKER_LEASE_SECONDS", DEFAULT_LEASE_SECONDS),
        poll_interval=num("KERT_WORKER_POLL_INTERVAL", DEFAULT_POLL_INTERVAL),
        reclaim_interval=num("KERT_WORKER_RECLAIM_INTERVAL", 5.0),
        max_jobs=integer("KERT_WORKER_MAX_JOBS", 0),
        backoff_base=num("KERT_WORKER_BACKOFF_BASE", DEFAULT_BACKOFF_BASE_SECONDS),
        backoff_factor=num("KERT_WORKER_BACKOFF_FACTOR", DEFAULT_BACKOFF_FACTOR),
        backoff_max=num("KERT_WORKER_BACKOFF_MAX", DEFAULT_BACKOFF_MAX_SECONDS),
    )
