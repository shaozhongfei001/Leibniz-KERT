"""客户经理持续经营 Skill 运行平台（KERT 工程内能力）。
...
- KERT 知识协作：执行前经本平台知识服务检索客户片段，注入知识上下文与证据引用（fail-open）。
...
【2026-09-12 注释更正】原文写作「两个独立 Skill：外联脚本 / 会面脚本
（R1 拜访报告已于 2026-08-21 下线移除）」，该表述与实现不符，现更正：
`skill-customer-previsit-report` 在 `registry()`（约 169 行）注册、
在 `execute()` 有专属分支（约 218 行，含 v1.3 无新证据策略
`exit_policy_no_new_evidence`）、在 executor 映射（约 542 行）与
`_run_previsit`（约 635 行）实现中完整存在，并被
`tests/integration/test_skills.py` 与 `tests/e2e/` 持续覆盖。
不存在"下线移除"动作：该注释系 2026-08-27 基线导入提交
`4a4991e` 一并带入，非一次移除变更的结果。
"""
...
class SkillExecutionService:
    def __init__(self, workspace: Path | None = None,
        ...
        ``KERT_LLM_REDACTION``，**默认开启**——客户数据进入外部模型属重大
        ...
        self._profile = (profile if profile is not None
                         else os.environ.get("KERT_PROFILE", "dev")).strip().lower()
        if llm_redaction is None:
            raw = os.environ.get("KERT_LLM_REDACTION", "").strip().lower()
            # 安全默认：未显式关闭即启用
        ...
        # v1.3：客户知识取数层（GITS 只传 customerId，知识全在 KERT）
    ...
    def _executor(self, skill_id: str):
        ...
        if skill_id == "SP-21":
            # Phase 3：交互记忆抽取（LLM + 确定性比对；KERT 不存记忆）
    ...
    def _trace_ki(trace: list[dict], ki_id: str, name: str, ok: bool) -> None:
        """逐知识条目取数步骤；v1.3：ok/skipped 只反映 KERT 知识库对该客户+KI 是否取到数。"""
        if ok:
            trace.append({
                "phase": "evidence", "status": "ok", "kiId": ki_id,
                "message": f"读取知识条目 {ki_id}（{name}），取数完成：KERT 知识库命中",
            })
        else:
            trace.append({
    ...
    def _run_outreach(self, request: dict, trace: list[dict]) -> tuple[dict, dict]:
        ...
        return {
            ...
            "evidenceRefs": [{"id": kid, "summary": f"{kid}（KERT 知识库）"} for kid in ki],
        }, model_call
    ...
    # ---------------- 供应链图谱（v1.3：只认 customerId，从 KERT 库构建） ----------------
    ...
    def sp20_gate_checklist(self, customer_id: str = "") -> list[dict]:
        """GATE-BIZ-* 闸门清单资产（权威清单在 KERT 资产，推进权威在 GITS）。"""
        from ..application.service_proposal import ServiceProposalExecutor
