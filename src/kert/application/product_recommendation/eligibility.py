...
def _as_date(value):
    ...
    支持 ISO 8601 完整格式（含时区），例如：
    - "2026-08-31T16:00:00Z" → UTC 16:00 = 北京时间次日 00:00 → 2026-09-01
    - "2026-09-01T00:00:00+08:00" → 北京时间 00:00 → 2026-09-01
    - "2026-09-01" → 直接解析

    对公业务默认时区 UTC+8（中国标准时间），UTC 时间按 +8 偏移取本地日期。
    """
    if value is None:
        return None
    ...
    if isinstance(value, str):
        ...
        if not s:
            return None
        # 先尝试完整 ISO 8601 解析（含时区）
        try:
            # Python < 3.11 不支持 Z 后缀，替换为 +00:00
            normalized = s.replace("Z", "+00:00")
            dt = _dt.datetime.fromisoformat(normalized)
            # 如果有时区信息且为 UTC（+00:00），按中国时区 +8 偏移取本地日期
            if dt.tzinfo is not None and dt.utcoffset() is not None:
                if dt.utcoffset().total_seconds() == 0:
                    # UTC 时间 → 加 8 小时得到中国本地日期
