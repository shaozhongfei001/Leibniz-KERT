"""
KERT Python Client — GITS Adapter 参考实现
...
class KertConnectionError(KertError):
    """网络连接失败"""


class KertTimeoutError(KertError):
    """请求超时"""


class KertSkillError(KertError):
    """Skill 执行错误（KERT 返回非 ok 状态）"""


class KertJobFailedError(KertError):
    """异步 Job 执行失败"""
...
class KertClient:
    ...
    用法::
        ...
        client = KertClient(base_url="http://127.0.0.1:8106", api_key="xxx")
    ...
    def poll_job(
        ...
        *,
        ...
        Raises:
            KertJobFailedError: Job 执行失败
            KertTimeoutError: 轮询超时
        """
    ...
    def _request(
        ...
        *,
        ...
        if last_exc:
            raise last_exc
        raise KertError("Unknown error after retries", error_code="RETRY_EXHAUSTED")
...
if __name__ == "__main__":
    ...
    base_url = os.environ.get("KERT_BASE_URL", "http://127.0.0.1:8106")
    api_key = os.environ.get("KERT_API_KEY", "")

    client = KertClient(base_url=base_url, api_key=api_key)
