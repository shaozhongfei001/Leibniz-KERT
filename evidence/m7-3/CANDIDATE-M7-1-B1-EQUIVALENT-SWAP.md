...
VERSION  : v1.11（2026-09-16：**E-9 提交时 sha 守门口径**入档（`sha256sum` 与 `git add` **同一命令内**完成；
           提交后必须打印**实际入库值** `git show HEAD:<path> | sha256sum`，不得把校验时的值当作记录）
           ＋记明其 TOCTOU 实证与"**首日即中止一次提交**"；并附 **E-5 附加条款**：
           `git cat-file -e <sha256>` 是**无效校验**（git 对象 id 为 SHA-1，与内容 sha256 不同族））
           v1.10（2026-09-16：**E-8 管道退出码口径**入档（管道后必须取 `${PIPESTATUS[0]}` 并与原始结果行一并打印；
           单测场景加"显式 0 failed"检查）＋记明本片采用**更强形态：根本不经管道**
           （`pytest … > /tmp/f 2>&1; echo $?`）；权威登记指向决策清单 E 段）
           v1.9（2026-09-16：**E-7 sha 可复现性**入档（整条取自命令输出；只读前 N 位就只能报前 N 位并标注；
           严禁补全尾部；64 位 sha 必须可被一条命令复现，否则视为未验证值）+ 不可复现 sha 实例的**据实记账**
           与"不改写历史、正向更正"处置；权威登记指向决策清单 E 段）
           v1.8（2026-09-16：**E-6 文档类交付协议**入档（改完再报定稿 / TL 不提交中间态）——
           本条与 v1.7 的 R-G 执行口径、E-5 跨轮规则**同一版定稿**提交）
           v1.7（2026-09-16：TL 本轮两项收紧 —— ① **R-G 升格为可执行口径**（主口径=显式 node id 集合
           **逐条比对**、禁用聚合值；双计数须**同一脚本同一次跑**；差异**归因不了即停下**）；
           ② **E-5 立为跨轮规则**（sha 取**入库值**；引用工作区值须**显式标注**；两者不一致须**同时给出并说明差异**）
           —— 两条均已写入 §6 第 7 项 / §10.1 前）
           v1.6（2026-09-16：B-1 **实施交付** + TL 定稿 —— ① **R-G**（探针口径缺口：探针只包 `_load_ki`，
           已接线技能的生产路径不再经过它 ⇒ 核对读取路径须**同时计数** `_load_ki_from_declaration`）
           落入 §6 第 7 项；② 新增 **§10 B-1 实施验证记录**（A-10 白名单授权行 / T6 表述修正 /
           `assetVersion` 双用途澄清 / 14-14 变异矩阵与**逐次红集** / 三条测量学陷阱）；
           ③ **sha 口径**改为以**入库 HEAD** 为准，报告前必须重取）
           v1.5（2026-09-16：TL 加固 R-A…R-F + 冻结**改判** —— ① A 组取**两跑交集**（R-A）
...
- **R-G（TL 定稿 2026-09-16；R-E 家族下一条 —— 探针口径缺口；含**可执行口径**）**：
  `/tmp/m71b_impact_probe.py` **只包装 `_load_ki`** ⇒ B-1 之后三个**已接线技能的生产路径不再经过它**，
  该探针对这三个技能的 `load_ki` 计数**结构性失效**（表现为"计数下降"，**不是**"读取变少"）。
  **执行口径（TL 收紧，三条；适用于**正式确认跑**及后续任何读取路径核对）**：
  1. **主口径 = `_route_plan` 命中用例集合**（不受本缺口影响），且**必须是显式 node id 集合逐条比对**
     —— **不得**以"命中数 44/44 一致"这类**聚合值**代替：聚合相等而集合不同（一进一出）**正是要抓的东西**；
  2. **双计数必须在同一脚本、同一次跑内产出**（一个探针两枚计数器：`_load_ki` + `_load_ki_from_declaration`），
     **不得**跑两次再拼接 —— 两次跑会引入 **R-A** 的 nodeid 归属竞态；
  3. 双计数差异若**归因不了 ⇒ 视为停下条件**（立即报 TL），**不得**以"记录为结构性差异"当作通过；
     差异可归因时须写明机制。
  **禁止**：仅凭该探针的 `load_ki` 计数论证"读取未被改动"（除上述"双计数"语境）—— 违反即视为**违规报告**。
  实测依据见 §10.3。
...
> **E-5（TL 立为**跨轮规则**，2026-09-16）｜sha 取数纪律（**双向消除歧义**）**：
> 1. 报告 sha **必须先取入库值**：`git show HEAD:<path> | sha256sum`，**不得沿用旧值**；
> 2. 引用**工作区值**时必须**显式标注**（写"工作区"），**不得**与入库值共用同一称谓；
> 3. 两者**不一致时必须同时给出并说明差异**（例：`HEAD=…；工作区=…，差异=未提交的 §10.7/R-G 增量`）。
>
> **理由（据实记账）**：本轮是 **TL 先拿错口径**（以**工作区值**比对交付方的 **HEAD 值**）才误判为"报数不符"
> ⇒ 规则必须**同时约束双方**，而不是只约束交付方。
> **落盘实例**：schema `5abfa6de…` **作废**（= 补 `assetVersion` 澄清**之前**的值）⇒ 入库值 `d8f9b6b6…`；
> 本文件自身在本段修改前：**HEAD 值 = 工作区值 = `2cf565d8…`（849 行）**。

> **E-6（TL 立，2026-09-16）｜文档类交付协议（消除"提交与编辑赛跑"）**：
> **交付方"改完再报定稿"** —— 全部编辑完成后，以 `定稿：<行数> / <sha256>` 形式**单次声明**（sha 按 **E-5** 取**工作区值**并**显式标注**）；
> **TL 不提交正在被编辑的中间态**，按该定稿声明提交。
> **理由（据实记账）**：本文件曾在 TL 两次提交之间走过 **849 → 861 → 865 行**，TL 追了三次中间态、并因此写出过陈旧行数
> ⇒ **单凭行数 / 旧 sha 引用必然过时**，故以"**单次定稿声明**"取代"边改边报"。
> 本条自身纳入**本轮定稿**一并提交（即：E-6 段落随其声明的定稿版本入库）。

> **E-7（TL 立，2026-09-16）｜sha 可复现性（**权威登记：决策清单 E 段**；本处为同一文本的引用，如有分歧以决策清单为准）**：
> **sha 必须整条取自命令输出**；用 `cut -c1-N` 只读了前 N 位时，报告中**也只能给前 N 位并显式标注"前 N 位"**，
> **严禁自行补全 / 转写尾部**；任何给出的 **64 位 sha 必须能被对方用一条命令复现**，否则视为**未验证值**。
>
> **实例（据实记账）**：`6e21022` 的提交信息与某一封信中曾出现**不可复现**的 64 位值
> （前 24 位真实、后 40 位系补全；`grep` 全仓**零命中** ⇒ **未污染任何仓内文件**）。
> 真实值 = `de090709afde8ab0ef254bbe3138afd47ece1a4d0e3f67e5bd7ad36d10db9802`（874 行，**工作区与 HEAD 三次一致**）。
> **处置：不改写历史**（不 `amend`），以**正向更正记录**对齐；本轮所有守门用的是 `skills.py == golden 1677173c…`，
> **与该错误值无关**，故**无功能影响**。
> 本条自身纳入**本轮定稿**一并提交。

> **E-8（TL 立，2026-09-16）｜管道退出码口径（**权威登记：决策清单 E 段**；本处为同一文本的引用）**：
> 管道后报"**退出码**"**必须取 `${PIPESTATUS[0]}`**，并**与原始结果行一并打印**；
> 单测场景另加**显式"0 failed"断言**（防"`passed` 数字看着对、其实 `failed` 前缀被截断"）。
> **实证（据实记账）**：另一工作包用 `python3 -m pytest … | tail -2` 却未取 `${PIPESTATUS[0]}` ⇒ 回传的是
> **管道末端 `tail` 的 0**，据此报出"1441 passed / 退出码 0"，与 TL 实测（`tests/unit` **3 failed** / exit 1）矛盾；
> 其当场自证机制（`piped_exit=0` vs `direct_exit=1`）并**撤回该声明**。
> **本片采用的更强形态（优选，≥ E-8）**：**根本不经管道** —— `pytest … > /tmp/f 2>&1; echo $?`，再 `grep` 读结果行；
> E-8 的"**原始结果行一并打印**"与"**0 failed 断言**"照旧执行（见 §10.2 与
> `evidence/m7-3/EVIDENCE-CONFIRM-RUN-M7-1-B1.md` §3）。

> **E-9（TL 立，2026-09-16）｜提交时 sha 守门口径（**权威登记：决策清单 E 段**；本处为同一文本的引用）**：
> **`sha256sum` 与 `git add` 必须在同一条命令内完成**；提交后**必须打印实际入库值**
> （`git show HEAD:<path> | sha256sum`），**不得把"校验时的值"当作记录**。
> **实证（据实记账，TOCTOU）**：本片证据入库 `7571594` 时守门为"先校验、后 `git add`"，
> 而两次操作之间交付方又改了一次（189 → 193 行）⇒ **校验值 ≠ 实际入库值**，提交信息里写的却是校验时的值
> ⇒ **提交信息与该提交自身内容不符**（无害，但属**记录失真**）。
> **首日即生效**：本条第一次执行就**中止了一次提交**（A 稳定、B 已变）⇒ 避免了同一失真的第二次发生。
> **配套（E-5 附加条款，权威登记同处）**：`git cat-file -e <sha256>` 判"内容是否入库"**是无效检查**
> —— 它接受 **git 对象 id（本仓 SHA-1）**，与内容 `sha256` **不同族** ⇒ 结果无意义；
> 正确做法即本条：`git show HEAD:<path> | sha256sum`。该陷阱比"未校验"更危险（会给人**已校验的错觉**）。

### 10.1 落地清单（入库于 B-1 提交 `c073c93`；开工前代码基线 `9b79da67`）
