...
VERSION  : v1.6（2026-09-16：B-1 **实施交付** + TL 定稿 —— ① **R-G**（探针口径缺口：探针只包 `_load_ki`，
...
> **取数纪律（TL 2026-09-16 提醒，已收紧）**：本节的 sha **一律取当前入库（HEAD）版本**；
> 交付后任何再次改动都会改 sha（实测：schema 因补 `assetVersion` 澄清一行 ⇒ `5abfa6de…` **作废**，
> 入库值为 `d8f9b6b6…`）。⇒ **报告 sha 前必须先 `git show HEAD:<path> | sha256sum`**，不得沿用旧值。
