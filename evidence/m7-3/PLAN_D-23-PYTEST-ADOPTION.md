...
## 6. 实施记录（2026-09-16；**回填**，E-3）

> 本节在**方案入库之后**回填（2026-09-16，TL 派工）。凡**方案原设计未含**、后由 **TL 追加要求**或**实现中新增**者，均在 §6.3 **逐条标注来源**，不混入方案原设计。

### 6.1 改动面与指纹（E-5 三重核对：HEAD blob == worktree）

| 文件 | numstat | sha256(16) before → after | 行数 |
|---|---|---|---|
| `tests/e2e/kert_api_validation.py`（legacy） | `15 15` | `b101fb83d656997b` → **`876db8b1754755b2`** | **588 → 588（不变）** |
| `tests/e2e/test_api_validation_adoption.py`（新增入口） | 新增 | — → **`1026751a7b8620c5`** | 0 → **144** |

- legacy 改动 = **7 处 `def` 改名 + 7 处调用点改名 + 1 行净零 docstring 备注**；`def test_` = **0**、`def check_*` = **7**，
  行号 **108 / 119 / 139 / 158 / 216 / 364 / 417** 与改前 `def test_*` **逐一致** ⇒ **既有 `:NNN` 引用全部仍有效**。
- **判据未动**：`biz_ok` 仍在 `:391` / `:395` / `:410`（TL 独立复核）。
- **入库态复核**：两文件 `HEAD blob == worktree`（`876db8b1754755b2` / `1026751a7b8620c5`）✓。
- `ruff`：两文件 `All checks passed!`。

### 6.2 §3 五项验收证据（原文）

1. **收集**（`-o addopts=""` 覆盖叠加的 `-qq`，否则 node id 列表被抑制）：目录级 `tests/e2e` **47 → 69（+22）**；
   新入口 **22 条**；**legacy 文件名未出现在收集结果** ✓。
2. **正常态（供给态活栈，专用端口 8141，CI 同口径）**
   `KERT_BASE_URL=http://127.0.0.1:8141 E2E_REQUIRE_KERT=1 E2E_LEDGER_PATH=… .venv/bin/python -m pytest tests/e2e/ -rs --junit-xml=…`
   ⇒ **`P_EXIT=0`**、`49 passed, 20 skipped in 5.96s`；junit `tests=69 failures=0 errors=0 skipped=20`；
   账本 `collected=69 / passed=49 / failed=0 / errors=0 / skipped=20 / violations=[]`，
   **`min_passed` 26 → 48（恰好 +22）**、`max_skipped=21`（≥ skipped=20）。
3. **变异自证（未供给栈，专用端口 8142）**
   `KERT_BASE_URL=http://127.0.0.1:8142 E2E_REQUIRE_KERT=1 E2E_LEDGER_PATH=… .venv/bin/python -m pytest tests/e2e/test_api_validation_adoption.py -rs --junit-xml=…`
   ⇒ **`U_EXIT=1`、2 failed / 20 passed**；两条 = `skill-customer-outreach-script`、`skill-customer-meeting-script`，
   报文 `HTTP 200；业务状态: skill_error (期望: ok)`；账本 `violations=['failed=2（必须 0）', 'passed=20 < 下限 22 …']`
   ⇒ **业务判据确实在跑、强度未降**（§3 预登记：若此处全绿即视为弱断言、必须停下报 TL —— **未发生**）。
4. **反向变异**：**按 §3 的"可选"未做** —— 需临时改动 legacy 判据，风险大于收益；③ 已足以证明业务判据在跑：
   `业务状态: skill_error (期望: ok)` 这条报文**只能**由 legacy 的 `biz_ok` 判据（`:388-396`）产生。
5. **账本**：见 ②③。

环境（E-11）：`.venv/bin/python` = **Python 3.12.8**、**pytest 9.1.1**；两条命令均**显式专用端口**并复核 `ss -ltn` 已释放。

### 6.3 相对方案的增量（**逐条标注来源**，非方案原设计）

| # | 增量 | 来源 | 说明 |
|---|---|---|---|
| 1 | `test_collection_contract` 的 **(b) 防复发断言**（`dir(kert_api_validation)` 内**不得存在 `test_*`**） | **TL 追加要求**（方案**入库之后**提出） | 封死"改名 / 加 `python_files` ⇒ 静默复活 7 ERROR"这条路 |
| 2 | 同函数的 **(a1) 条件化收集下限（≥37）** 及"**非静默**声明" | **TL 追加要求** + 实现中定型 | 该下限**仅当本次 session 覆盖到其它 e2e 文件时判定**；单文件调用不适用 ⇒ 已在代码注释与本节**显式声明**，**不静默** |
| 3 | legacy 模块 **1 行净零 docstring 备注**（"脚本形态 / `check_*` / pytest 不收集 / 用例入口"） | **实现中新增**（§2.3 未含） | **净零**：只**替换既有空行**，不增行（见 §6.4） |
| 4 | 实现文件指纹（本入口 **144 行 / `1026751a7b8620c5`**） | **实现产物** | 方案 §2.2 只有签名骨架，无指纹 |

- 另有**实测口径**补充：目录级真实收集 = **69**（不止 15+22 —— 另含 8 个 scenario / cross-service 文件共 47 条）；
  其与 (a1) 的 **37** 之差**已由 TL 裁定**（见 §6.5）。

### 6.4 一处自纠（实现中自查并修正；E-2/E-12 同族）

**初版 docstring 备注写法使文件净 +1 行** ⇒ 会让**其后所有行号下移**，使既有 `:NNN` 引用
（含本方案 §1 的 `:131-135`、`:384-410`、`:445-467`，以及清单 D-22 行内引用的该文件行号）**全部失效**。
⇒ 已改为**净零替换**（把**既有空行**替换为备注行）⇒ 回填后 legacy 仍 **588 行**、7 个 `def` 行号与改前**逐一致**。
⇒ 该族风险登记为自我约束：**凡改动"被行号引用"的文件，必须同轮核对 `wc -l` 与关键定义行号，或改用净零替换**
（与 E-2 双锚同族）。

### 6.5 TL 裁定：(a1) 下限**维持「37 + 本文件 22」**，不抬到 69

理由（TL，2026-09-16）：**37 是锚定入库态的低稳定下限**（**E-12** 口径）；
**69 会被无关 e2e 文件的增删、以及解释器/依赖差异牵动而假红**（此前 system-python 那次即此类）；
"无关文件静默丢失"这件事**由 conftest 账本 `min_passed` / `max_skipped` 承担**（已自动 +22）。
⇒ (a1) 的**条件化判定必须保持"非静默"**（已满足：代码注释 + §6.3 表格**双重声明**）。

### 6.6 非声明

本节记录的是**实施结果**，**不等于** QA_PASS / 验收通过 / 生产就绪；**不代表** CI 已跑过该入口
（上表为**本地活栈**实测，非 CI 实跑）；**不代表** D-23 已关闭。
本实施产物由 **TL 入库**；**c20 侧未自行 `commit` / `push`**。
本轮**未**修改 `tests/e2e/**` 之外的文件（未动 `specs/**`、`src/**`、`deploy/**`、`docs/**`）。
本节所引行号均为**回填时点**（legacy **588 行 / `876db8b1754755b2`**）行号，后续改动须按 E-2 重新对齐。
