...
| **A-8** | **锚点刷新轮（D-8 落地）**：`server.py` 因本批净 **+28 行** ⇒ 冻结件中旧行号引用需刷新：合同 **14 处** + 测试注释 **7 处**；`evidence/m7-3/CANDIDATE-*.md` / `EVIDENCE-*.md` / 本决策清单属**带日期的快照**，**旧行号保留不改** | TL **已授权但设门**：**等 m71 冻结态复测结束**再开工（否则又制造"并发期跑数不可归因"）。附加要求：① 零语义（纯文本）② 附**扫描证据**（`grep -rn "server\.py:[0-9]"` 全量输出 + 显式豁免清单）③ 开工前确认**无任何用例/脚本对 `specs/**` 做内容哈希**（文本改动会连锁改哈希）④ 报新 sha | 引用可核性（不影响功能） |
