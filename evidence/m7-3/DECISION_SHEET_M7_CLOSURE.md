...
| **A-7** | **1.5.1 追认对象**：合同已 bump `1.5.0 → 1.5.1`（形状/错误响应更正），提交于 `e3bcefe` | **待你追认**：确认追认覆盖 v1.5 的 **additive 增量**、且 `1.5.1` 为**形状更正补丁**（语义不变）。同时 `1.5.1` 已含 F4b/F4d/F4e 三处形状更正（execute 400→422、report 404 字符串 detail、maps 500 `oneOf`） ；**追认对象 = 勘误后 `9c9994026fa62fff…`（1821 行）**（c20 与 TL 双核） | 追认对象的唯一性 |
