...
```text
subClassOf 新增 = 101，拆解为：
  · 自反(s==o)**全量** = 34  ← 本报告口径
      其中 本域主语 = **32**（= TL 复算的"本域 32"），另 2 条为 owl:Thing / owl:Nothing 顶层公理（非本域）
  · 本域主语、**非自反** = 32，其宾语**全部是 `owl:Thing`** ⇒ 平凡顶层公理（噪声）
  · **两端皆本域**、非自反 = **0**  ← "有意义的层次闭包为 0" 的**确切口径**（TL 的"非自反=0"即此）
⇒ 34 / 32 / 0 三个数同源且各自成立，差别只在**是否限定"两端皆本域"**与"是否含顶层公理"。
```
```
