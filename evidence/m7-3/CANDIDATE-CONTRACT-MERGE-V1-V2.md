...
| D3 | `docs/contracts/schemas/*.json` 与 v1 内联 schema 的关系 | 保留为**细节 canonical 层**，v1 以引用方式指向（`assembly-trace` 已是先例），**不**与 v1 并列成为第二权威 | v1（见清单 §3 [D-16-A]） 已明确"权威定义见 schemas/assembly-trace.schema.json，本处不复写"；README:14-15 同口径 |
...
| OpenAPI 方言 | `3.0.3`（v1（见清单 §3 [D-16-A]）） | `3.1.0`（v2:1） |
| `info.version` | `1.5.0`（v1（见清单 §3 [D-16-A]））→ **已于 2026-09-16 更正为 `1.5.1`** | `2.0.0-candidate`（v2:4） |
| `servers[0].url` | `http://127.0.0.1:8106`（v1（见清单 §3 [D-16-A]）） | `http://127.0.0.1:8106`（v2:11，描述"实际以部署配置为准"） |
| 路径数 | **14**（v1:42,72,95,130,229,255,299,331,400,418,438,452,472,498） | **10**（v2:28,41,91,115,139,157,177,195,203,213） | （基准：勘误前 1826 行版（1826 行）；重定位见清单 §3 [D-16-A]）
| schema 数 | **36**（更正记录：原文写 30，系笔误——未逐个数 `components.schemas`；实测 36） | 17（v2:257-567） |
| 顶层 `security` | **无**（仅定义 schemes，v1（见清单 §3 [D-16-A]）） | **有**全局要求（v2:13-14） |
...
| # | 路径 | v1 | v2 | 判定 |
|---|---|---|---|---|
| 1 | `/api/skill/health` | v1（见清单 §3 [D-16-A]），`operationId: getSkillHealth` | v2:28-40，`operationId: listSkills` | **operationId 冲突**（见下 B-①） |
| 2 | `/api/skill/execute` | v1:130-227（**该区间含已由 A-9 删除的 `/v1/skills` 端块（旧 130-164）** ⇒ 原跨度失效、**不得重定位**；见清单 §3 [D-16-A]） | v2:41-90 | 路径同、状态码集不同（§2.3） |
| 3 | `/v1/jobs/{jobId}` | v1（见清单 §3 [D-16-A]） | v2:91-114 | 路径同、响应 schema 不同（§2.2-④） |
| 4 | `/api/skill/gates/{customerId}` | v1（见清单 §3 [D-16-A]），`operationId: getGates` | v2:139-156，`operationId: listGates` | **schema 结构性冲突**（§2.2-②） |
| 5 | `/api/skill/gates/audit` | v1（见清单 §3 [D-16-A]），`operationId: auditGate` | v2:157-176，`operationId: recordGateAudit` | 响应 schema 不同（§2.2-③） |
| 6 | `/livez` `/readyz` `/metrics` | v1（见清单 §3 [D-16-A]）（`livez`/`readyz`/`getMetrics`） | v2:195-220（`liveness`/`readiness`/`metrics`） | operationId 全不一致（§2.3-D） |

**B. v1 独有（8 条）**

| # | 路径 | v1 行号 | 实现 | 说明 |
|---|---|---|---|---|
| ① | `GET /v1/health` | v1（见清单 §3 [D-16-A]） | `server.py:448-469` ✅ | GITS 未调用；v2 未声明 |
| ② | `GET /v1/skills` | v1（见清单 §3 [D-16-A]） | **未实现** | `docs/integration/KERT_GITS_CONTRACT_DIFF.md:14`（"规范要求新增，server.py 尚未实现"）、`:96`（P2 待做）。**v1 自身的"幽灵路径"** |
| ③ | `GET /api/skill/report/{requestId}` | v1:229-253（**跨界**：229-235 段未位移、236-253 段因勘误下移 ⇒ 不得只给单一新值；重定位后 = 473-492，基准 = 勘误前 1826 行版） | `server.py:645-658` ✅ | **GITS 活动调用**（`KERT_GITS_CONTRACT_DIFF.md:16`；GITS `application.yaml:56`；`examples/gits_adapter/README.md:164`）。**v2 完全未声明 → 高危** |
| ④ | `GET /v1/knowledge-maps` | v1（见清单 §3 [D-16-A]） | `server.py:693-713` ✅ | v1.5 追认增量（§0.1 追认项 ①）；v2 未声明 |
| ⑤ | `GET /v1/knowledge-maps/{mapId}` | v1（见清单 §3 [D-16-A]） | `server.py:715-727` ✅ | 同上 |
| ⑥ | `POST /v1/routing/plan` | v1（见清单 §3 [D-16-A]） | `server.py:729-747` ✅ | 同上 |
...
- `listSkills`：v1 用于 `GET /v1/skills`（v1（见清单 §3 [D-16-A]）），v2 用于 `GET /api/skill/health`（v2:30）。
...
|---|---|---|---|---|
| ① | **`SkillHealthResponse`** | v1（见清单 §3 [D-16-A]），`required: [status, skills]`，**无 `service`** | v2:257-272，`required: [status, service, skills]`，`service` 注 `x-note: 生产版应配置化，不写死 customer-engagement`（v2:266） | **v2 更贴合实现**：实现返回 `{status, service:"customer-engagement", skills}`（`server.py:612-617`）。v1 因未 `additionalProperties:false` 而"恰好不报错"，但 v1 未登记 `service`（撞 C-14 同源问题） |
| ② | **`GateListResponse.gates[]`** | v1（见清单 §3 [D-16-A]） → `GateChecklistItem`（v1（见清单 §3 [D-16-A]））：`{gate, state, name, checklist:{must[],forbidden[]}}`；示例 v1（见清单 §3 [D-16-A]） 亦为 `gate: G0 / state: PASSED / checklist.must` | v2:458-467 → `GateDefinition`（v2:469-492）：`required [gateId,name,sequence,must,forbidden]` + `assetPath/version/contentHash`；`schemas/gate.schema.json:6-45` 同形且 `additionalProperties:false`（`:45`） | **结构性冲突，且 v2 正确**：实现返回 `{"gateId","name","sequence","must","forbidden","assetPath"}`（`src/kert/application/service_proposal.py:114-117`，经 `skills.py:890-896` → `server.py:660-663`）。**v1 的 `gate`/`state`/`checklist` 三个键在实现中不存在** |
| ③ | **`GateAuditResponse`** | v1（见清单 §3 [D-16-A]）：`{recorded, timestamp, auditPath}` | v2:509-519：`{recorded, auditId, recordedAt}` | **两者都与实现不符**：实现返回 `{"recorded":true, customerId, gate, decision, decidedBy, reason, recordedAt}`（`skills.py:907-920`）。v1 的 `timestamp`/`auditPath` 不存在；v2 的 `auditId` 不存在；`recordedAt` 在 v2 ✅ |
| ④ | **Job 状态响应** | v1（见清单 §3 [D-16-A]） `JobStatusResponse`：`required [jobId, status]`，`jobId`/`createdAt`/`startedAt`/`completedAt`，`data.skill_result`（v1（见清单 §3 [D-16-A]），`description: COMPLETED 时包含 skill_result`） | v2:362-374 `JobResponse`：`required [jobId, status]`，**顶层** `skill_result` + `error`；`schemas/job.schema.json:25-27`（`skill_result`）、`:14-24`（status 6 值含 `DEAD`/`CANCELLED`） | **`skill_result` 位置不一致**：v1 在 `data.skill_result`，v2 在顶层。**GITS 现读 `data.skill_result`**（`gits-cbanking/.../DshJobPoller.java:122-126`）→ v2 采用即破坏轮询器（另见 §2.5-G） |
| ⑤ | **错误信封** | v1（见清单 §3 [D-16-A]） `ErrorResponse`：`required [requestId, status]` + `status enum [skill_error, exit_policy_no_new_evidence]`（v1（见清单 §3 [D-16-A]））+ `errors[]` | v2:434-443 `ErrorResponse`：`required [error]`，`{error: ErrorItem, requestId, traceId}`；`ErrorItem`（v2:445-456）`required [code, message]` + `retryable`/`details` | **互斥的两种信封**，见 §2.3 |
| ⑥ | **错误明细字段名** | v1（见清单 §3 [D-16-A]） `ErrorDetail`：`code` / `message` / **`detail`**（v1（见清单 §3 [D-16-A]）） | `schemas/error.schema.json:10-22`：`code` / `message` / **`retryable`** / **`details`**；v2:445-456 同名 | **同概念两名**：`detail` vs `details`；v1 无 `retryable`。实现用的是 `retryable`（`server.py:311-313`）→ 与 JSON Schema/v2 一致，与 v1 不一致 |
| ⑦ | **`SkillExecuteResponse` 必填集** | v1（见清单 §3 [D-16-A]）：`required [requestId, status, data]`（`errors`/`assemblyTrace`/`modelCalls` 可选） | `schemas/skill-execute-response.schema.json:6-13`：`required` **6 项全必填**；`:49` `additionalProperties:false` | **严格度冲突**：JSON Schema 比 v1 严。实现必发 6 项中的哪些未在 v1 声明，故按 v1 校验通过、按 schema 校验亦通过（当前实现确实都发），但**契约强度不等价** |
| ⑧ | **`SkillExecuteRequest.required`** | v1（见清单 §3 [D-16-A]）：`required [skillId, requestId, request]` | v2:285-300：`required [skillId]`（`request` 降为可选 `object`）；`schemas/skill-execute-request.schema.json:6-8` 同 v2，且 `:27 additionalProperties:false` | **必填集冲突**：v1 要求 3 项，v2/schema 只要求 1 项。更强的约束在 v1（对 GITS 无影响，GITS 全发，`DshHttpSkillExecutionAdapter.java:29`） |
| ⑨ | **`ContextPackage`** | v1（见清单 §3 [D-16-A]）：`required [schemaVersion, customerId]`，含 `gateState`（v1（见清单 §3 [D-16-A]））、`proposalContext`（v1（见清单 §3 [D-16-A]）） | v2:302-325 与 `schemas/context-package.schema.json:7-40`：含 `customerName`/`industry`/`enterpriseData`/`interactionContent`/`existingMemories`，`required: []`（schema `:39`），`additionalProperties:true`（`:40`） | **并集关系**：v1 有 `gateState`（v2/schema 无）、v2 有 5 个字段（v1 无）。且 schema 自述"权威 GITS 附录待对齐"（`context-package.schema.json:6`）= 登记在册的缺口（`docs/contracts/README.md:39`） |
| ⑩ | **`ServiceResult`（SP-20）** | v1（见清单 §3 [D-16-A]）：**无** `ruleViolations` | `schemas/sp20.schema.json:60-65`：**有** `ruleViolations`，且 `:67 additionalProperties:false` | v1 把 `ruleViolations` 放在 `SkillExecuteData`（v1（见清单 §3 [D-16-A]）），v2 语义下它在 SP-20 结果内。**位置不同**；`additionalProperties:false` 使该差异成为硬冲突（SP-20 结果带 ruleViolations 时按 sp20.schema 是合规的） |
| ⑪ | **`assemblyTrace` 条目** | v1（见清单 §3 [D-16-A]）：`type: array`（v1.5 由 `object` 修正而来，v1（见清单 §3 [D-16-A]）），元素为 `object` + `additionalProperties:true`，**不 `$ref`**，仅在 prose 指向 canonical schema（v1（见清单 §3 [D-16-A]）） | `schemas/assembly-trace.schema.json`：`required [phase,status,message]`（`:6-10`），含 v1.5 新字段 `mapId`/`mapExpected`/`mapMismatch`/`planHash`/`versions`（`:66-91`），`additionalProperties:true`（`:107`） | **已是"引用式"协同的先例**（READY 模式）：v1 不复写字段，canonical 在 schema。**这正是 D3 推荐的形态** |
| ⑫ | **`nullable` 语法** | v1 使用 `nullable: true` **8 处**：v1:1172,1183,1186,1189,1192,1244,1259,1280（另有 `deprecated: true` v1:662） | v2 无 `nullable`；`schemas/job.schema.json:42-47` 用 `type: ["string","null"]` | **方言冲突**：`nullable` 是 OpenAPI 3.0 关键字，**3.1.0 已移除**（3.1 用 JSON Schema 的 `type` 数组/null）。直接决定 D4 的代价（§3.1 P2） | （基准：勘误前 1826 行版（1826 行）；重定位见清单 §3 [D-16-A]）

### 2.3 错误码 / 状态码级差异

| # | 对象 | v1 | v2 | 判定 |
|---|---|---|---|---|
| A | `/api/skill/execute` 状态码集 | **200 / 202 / 400 / 404 / 500**（v1:175,189,198,210,222） | **200 / 202 / 401 / 403 / 404 / 409 / 413 / 422 / 429**（v2:53,59,65,67,69,75,81,83,89） | **互不包含**：v1 有 400/500 而无 401/403/409/413/429；v2 反之。归并必须取并集并逐条定裁决（`409 幂等冲突`、`413 体积`、`429 限流`在实现中确有对应中间件——`server.py:218-228` 的加固栈描述） | （基准：勘误前 1826 行版（1826 行）；重定位见清单 §3 [D-16-A]）
| B | 错误体结构（同状态码下） | `{requestId, status, errors[{code,message,detail}]}`（v1（见清单 §3 [D-16-A]）,1101-1127） | `{error:{code,message,retryable,details}}`（v2:434-456） | **不可兼容的两种结构**。实况是**两者并存**：`/api/skill/execute` 走 v1 式（`server.py:635-643`，来自 `result.as_dict()`），基础设施异常走 v2 式（`server.py:308-316` `_handle()` → `detail={"error":{code,message,retryable}}`） |
...
| D | 探针 operationId | `livez`/`readyz`/`getMetrics`（v1:405,423,443） | `liveness`/`readiness`/`metrics`（v2:197,205,215） | 需择一并登记为 deprecation（无外部消费方，`KERT_GITS_CONTRACT_DIFF.md:21-23` 标"KERT 独有"） | （基准：勘误前 1826 行版（1826 行）；重定位见清单 §3 [D-16-A]）

### 2.4 鉴权级差异

| # | 对象 | v1 | v2 | 实现（第三轴） | 判定 |
|---|---|---|---|---|---|
| A | 顶层 `security` | **无**（v1 只在 v1（见清单 §3 [D-16-A]） 定义 `ApiKeyAuth`，未在任何 operation 或全局启用） | **全局强制** `security: [ApiKeyAuth: []]`（v2:13-14） | `AuthConfig.enabled` 默认 **False**（`src/kert/infrastructure/runtime_config.py:81`） | v1 与实现一致（默认不强制）；v2 是**意图**（生产强制）。差异性质 = "契约声明"vs"当前实况"，须由 Owner 择一表达方式 |
| B | scheme 语义 | `X-API-Key`，描述"**演示环境可省略**"（v1（见清单 §3 [D-16-A]）） | `X-API-Key`，描述"KERT **服务到服务** API Key"（v2:224-228） | 实现：`header_name` 默认 `X-API-Key`，可配 `KERT_AUTH_HEADER`（`runtime_config.py:82`, `:396-404`） | 同名同位置；**描述语义不同**（"可省略" vs "服务到服务"），影响安全评审口径 |
...
| # | 发现 | 证据 | 影响 |
|---|---|---|---|
| F1 | `/api/skill/gates/{customerId}` 的 v1 形状（`gate`/`state`/`checklist`）**与实现不符**；v2 形状（`gateId`/`sequence`/`must`）**与实现相符** | v1（见清单 §3 [D-16-A]）,277-295 vs `service_proposal.py:114-117` vs v2:469-492 | **v1 有实况失实**（与 `assemblyTrace` 同类，属可修正缺陷） |
| F2 | `/v1/routing/plan` 的 v1 `RoutingPlanResponse` 要求**顶层** `allowed`（`required` 含之，v1（见清单 §3 [D-16-A]）），实现把 `allowed` 放在 **`data` 内** | v1（见清单 §3 [D-16-A]） vs `server.py:742-747`（`_response(..., data)`）+ `server.py:206-215`（信封）vs `tests/integration/test_routing_api.py:115-116,151-152`（`["data"]["allowed"]`） | **v1 在 v1.5 新端点上即与实现不符**（测试按 `data.allowed` 断言，故测试绿而合同错）。**这是本次最被低估的缺陷** |
...
| F4 | `/api/skill/execute` 的 404 响应体**不是** `ErrorResponse`，而是**与 200 同构**的 skill 结果（`status=skill_error` + `errors[0].code=UNKNOWN_SKILL`） | `server.py:635-643`（`status_code=404 if unknown else 200`，同一 `payload`）；GITS 亦按此实现（`DshHttpSkillExecutionAdapter.java:32`："未知 skillId → 404, errors[0].code=UNKNOWN_SKILL"） | v1（见清单 §3 [D-16-A]） 与 v2:69-74 都声明 404 返回 `ErrorResponse` → **两份都失实**（GITS 已按真实行为编码） |
| F5 | `/api/skill/gates/audit` 响应与 v1/v2 均不符 | `skills.py:907-920`（`{recorded, customerId, gate, decision, decidedBy, reason, recordedAt}`）vs v1（见清单 §3 [D-16-A]） / v2:509-519 | 见 §2.2-③ |
| F6 | 错误 `retryable` 字段：实现发 `retryable`（v2/schema 有，v1 无） | `server.py:311-313` vs v1（见清单 §3 [D-16-A]） vs `error.schema.json:17-19` | 支撑 §2.3-B/⑥ |
| F7 | `/api/skill/health` 实现返回 `service`（v2 有，v1 无） | `server.py:612-617` vs v1（见清单 §3 [D-16-A]） vs v2:257-272 | 支撑 §2.2-①；与 C-14（`service` 写死 `customer-engagement`）同源 |
| F8 | **v1 声明的 `/v1/skills` 未实现**；**实现有 9 条 `/v1/*` 未登记进 v1** | v1（见清单 §3 [D-16-A]） vs `server.py:4-17`（自述清单）与 `:471-605`（实际路由）；`KERT_GITS_CONTRACT_DIFF.md:14,96` | 双向覆盖缺口；归并须以"实现路由表"为基准做一次全量对账 |
...
| 端口 | GITS 默认 `DSH_BASE_URL=http://127.0.0.1:8107`，与两份合同 `servers` 的 `8106` **不一致** | `gits-cbanking/.../application.yaml:51` vs v1（见清单 §3 [D-16-A]） / v2:11（另见 KERT 仓根 `p24_serve_8107.py`） |
...
1. **单一权威**：`specs/kert-openapi-v1.yaml`（升级为 `1.6.0`，见 P1）。v1 的 `info.description`
   保留"运行中权威"表述（v1（见清单 §3 [D-16-A]）），并把"v2 候选未批准"改为"v2 已归档为非权威设计输入"。
2. **路径收敛**：
...
3. **schema 收敛**：
   ...
   - **`docs/contracts/schemas/*.json` 的定位**：保留为"字段级 canonical 细节层"，
     v1 以 **prose 引用 + 可选 `x-canonical-ref`** 指向（`assemblyTrace` 已是既有先例，v1（见清单 §3 [D-16-A]）），
...
4. **`scripts/validate_contract_bundle.py` 校验对象**：
   ...
   - 但 `:75-76` 硬要求 `openapi == "3.1.0"`，而 v1 是 `3.0.3`（v1（见清单 §3 [D-16-A]））且用了 **8 处 `nullable`**
     （v1:1172,1183,1186,1189,1192,1244,1259,1280）——`nullable` 在 3.1 中**不存在**。 （基准：勘误前 1826 行版（1826 行）；重定位见清单 §3 [D-16-A]）
     故 P2 只有两种实现方式：
...
|---|---|---|---|
| `POST /api/skill/execute` | 顶层 `requestId`/`status`/`data`/`errors`（`DshHttpSkillExecutionAdapter.java:187-197`） | 保留 v1 信封（v1（见清单 §3 [D-16-A]）） | 无 |
| `POST /api/skill/execute`（async） | 202 `{jobId,status}` | 保留（v1（见清单 §3 [D-16-A]）；`server.py:631-633`） | 无 |
...
- **GITS 可选做**：把 `DSH_BASE_URL` 默认值与合同 `servers`（8106）对齐，或在合同 `servers` 中
  改为配置占位（当前 v1（见清单 §3 [D-16-A]） / v2:11 均写死 8106，而 GITS 默认 8107，冲突 C-04 同源）。
...
| 方 | 期望 | 证据 |
|---|---|---|
| v1 合同 | 顶层 `jobId`（`required`），`data.skill_result` | v1（见清单 §3 [D-16-A]） |
...
| R5 | **OpenAPI 方言迁移引入语义漂移** | v1 升 3.1.0 后 8 处 `nullable` 改写（v1:1172…1280） | 中 | 逐处 diff 复核 + 用现有测试（`pytest tests/integration`）做回归；不做"批量替换" | （基准：勘误前 1826 行版（1826 行）；重定位见清单 §3 [D-16-A]）
