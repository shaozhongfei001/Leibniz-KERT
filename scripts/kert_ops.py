...
用法示例::
    ...
    # 校验
    python scripts/kert_ops.py verify --backup /var/backups/kert/backup-20260827T120000Z

    # 恢复（演练建议恢复到新目录，勿直接覆盖生产）
    python scripts/kert_ops.py restore \
        --backup /var/backups/kert/backup-20260827T120000Z \
        --target /tmp/restore-drill

    # 发布清单
    python scripts/kert_ops.py manifest --out evidence/release/RELEASE_MANIFEST.json
...
def cmd_backup(args: argparse.Namespace) -> int:
    """创建备份。"""
    from kert.domain.errors import KERTException
...
def cmd_verify(args: argparse.Namespace) -> int:
    """校验备份集完整性。"""
    from kert.infrastructure.backup import verify_backup
...
def cmd_manifest(args: argparse.Namespace) -> int:
    """生成发布清单。"""
    from kert.infrastructure.release import write_release_manifest
...
def cmd_compare(args: argparse.Namespace) -> int:
    """比对两份发布清单。"""
    from kert.infrastructure.release import compare_manifests
...
def build_parser() -> argparse.ArgumentParser:
    """构造命令行解析器。"""
    ap = argparse.ArgumentParser(description="KERT 备份/恢复/发布清单工具")
