...
def main() -> None:
    ...
    if not os.environ.get("KERT_LLM_BASE_URL"):
        try:
            import yaml
            ...
            if key:
                ...
                os.environ["KERT_LLM_MODEL"] = "deepseek-chat"
    ...
    from kert.infrastructure.runtime_config import ConfigError, load_runtime_config

    # M2.1/ADR-015：绑定地址纳入配置校验；生产 profile 缺少认证/限流时拒绝启动
    os.environ.setdefault("KERT_BIND_HOST", args.host)
    try:
    ...
    if not cfg.auth.enabled:
        print("[security][WARN] 未启用 API Key 认证（dev 模式）；"
              "生产环境请设置 KERT_PROFILE=prod 与 KERT_API_KEYS", flush=True)
    else:
