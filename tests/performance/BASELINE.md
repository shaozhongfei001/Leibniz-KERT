...
### 9.8 ⚠ 族 B 比值档 **K 3 → 10**（2026-09-17，CI 实测更正；两套标定模型都试过）

**触发**：develop 合并提交上 `Performance Benchmarks` 红一次，失败断言来自族 B 的**比值档**：

```
并发 job 写入 100 条 总耗时 403ms 超过比值档预算 378ms
（比值档：3.0× 100 条 × 参考负载 1.26ms/op）—— ≥3× 级回归
```

**两个模型都实测过，结论是"对争用受限的工作负载，墙钟比值不可标定"**：

| 分母模型 | 本地（20 核） | CI（低算力 runner） | 判定 |
|---|---|---|---|
单线程 per-op × ops（原用） | 0.73–1.35 | **3.2**（403ms / 378ms） | **不可移植**：并发开销占单线程成本的比例随核数/调度变化 |
**同竞争形状**标定（10 线程各写 10 条，本批试过） | **24.9**（burst 1491ms / 标定 60ms） | — | **不可用**：镜像负载**低估** `RuntimeStore` 每 op 的真实成本（每 op 多出 schema/锁/WAL 等开销） |

⇒ **处置**：K **3 → 10**（对 CI 实测 3.2 留 **3.1× 余量**），并把口径与依据写在测试常量处。
**灵敏度相应降为"≥10× 级回归"**（不变量与 12s 灾难上限不变）。
**残留风险（明示）**：若将来 CI 出现 >10× 的比值，**不得**再调 K —— 那时唯一诚实的做法是
**撤掉比值档、只留不变量 + 灾难上限**（并在本节登记），因为"给不可标定的量继续调参"就是赌运气。

**同批未动**：§9.7 的 NFR 缺口登记不变；lifecycle（族 A）的 K=3 **保持**（其比值在三态下实测稳定 0.81–1.32，
与族 B 的争用受限性质不同）。
