...
def test_env_keys_enable_auth_implicitly():
    """仅提供 KERT_API_KEYS 即隐式启用认证。"""
    cfg = load_runtime_config(env={"KERT_API_KEYS": f"svc-a:{STRONG_KEY}"})
...
def test_key_digest_not_plaintext():
    """配置中只保留摘要，不留明文密钥。"""
    cfg = load_runtime_config(env={"KERT_API_KEYS": STRONG_KEY})
...
def test_scopes_parsed_from_spec():
    ...
    cfg = load_runtime_config(
        env={"KERT_API_KEYS": f"admin-key:{STRONG_KEY}:admin|read"})
...
def test_weak_key_rejected():
    ...
    with pytest.raises(ConfigError, match="长度不足"):
        load_runtime_config(env={"KERT_API_KEYS": "short"})


def test_invalid_profile_rejected():
    """非法 profile 值直接报错。"""
    with pytest.raises(ConfigError, match="KERT_PROFILE"):
        load_runtime_config(env={"KERT_PROFILE": "staging"})
...
def test_prod_profile_without_auth_fails_fast():
    ...
    with pytest.raises(ConfigError, match="必须启用 API Key 认证"):
        load_runtime_config(env={"KERT_PROFILE": "prod"})
...
def test_prod_profile_complete_config_passes():
    ...
    cfg = load_runtime_config(env={
        "KERT_PROFILE": "prod",
        "KERT_API_KEYS": f"svc:{STRONG_KEY}",
...
def test_non_strict_mode_collects_warnings_instead_of_raising():
    ...
    cfg = load_runtime_config(env={"KERT_PROFILE": "prod"}, strict=False)
    assert cfg.is_production
...
def test_auth_enabled_without_keys_is_a_problem():
    ...
    assert problems == []
    cfg2 = load_runtime_config(env={"KERT_AUTH_ENABLED": "true"}, strict=False)
...
def test_config_file_digest_form(tmp_path):
    ...
    cfg = load_runtime_config(env={"KERT_CONFIG_FILE": str(cfg_file)})
    assert cfg.auth.verify(STRONG_KEY).key_id == "svc-x"


def test_config_file_missing_raises(tmp_path):
    """KERT_CONFIG_FILE 指向不存在文件时报错。"""
    with pytest.raises(ConfigError, match="不存在"):
        load_runtime_config(env={"KERT_CONFIG_FILE": str(tmp_path / "nope.json")})
...
def test_inactive_key_not_accepted(tmp_path):
    ...
    cfg = load_runtime_config(env={"KERT_CONFIG_FILE": str(cfg_file)}, strict=False)
...
def test_size_and_concurrency_env_parsing():
    ...
    cfg = load_runtime_config(env={
        ...
        "KERT_MAX_RESPONSE_BYTES": "4096",
        "KERT_CONCURRENCY_ENABLED": "true",
        "KERT_MAX_IN_FLIGHT": "3",
    })
...
def test_invalid_int_env_rejected():
    ...
    with pytest.raises(ConfigError, match="需为整数"):
        load_runtime_config(env={"KERT_MAX_IN_FLIGHT": "many"})


def test_runtime_store_path_defaults_to_control_dir(tmp_path):
    """Runtime Store 默认落在 90_control/runtime 下。"""
    cfg = load_runtime_config(env={"KERT_RUNTIME_STORE_ENABLED": "true"})
...
def test_rate_limit_bucket_math():
    """限流配置换算为桶容量与补充速率。"""
    cfg = load_runtime_config(env={"KERT_RATE_LIMIT_ENABLED": "true",
                                   "KERT_RATE_LIMIT_RPM": "120",
                                   "KERT_RATE_LIMIT_BURST": "5"})
