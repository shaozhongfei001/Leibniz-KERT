...
| B-06 | GITS 当前未实际调用 KERT | 形成 A+B 交接；保持 UAT_PASS=NO | GITS_KERT_A_PLUS_B_HANDOFF.md | OPEN（需 GITS commit/diff/UAT） |
...
- B-04: KERT/GITS 双方同一 contract hash 与 consumer contract test 报告。
- B-05: 受控实现审计包 + 独立 QA 复跑。
- B-06: GITS→KERT 真实 E2E + 故障空态 + Owner UAT。

## 当前未关闭 Major

M-01 至 M-16 已在独立评审报告中列出。Phase 0 在设计中覆盖了 M-01~M-15 的设计/基线部分；M-16 需要 GITS/KERT 业务事实冲突优先级合同，仍待契约 v2 与 GITS 对齐。
