...
| 控制项 | 内容 |
|---|---|
| 文档编号 | `KERT-ARCH-REVIEW-2026-08-26-002` |
| 文档版本 | `V1.0` |
| 评审日期 | `2026-08-26` |
| 评审角色 | Independent Enterprise Architecture Reviewer / Architecture QA Gate |
| 评审对象 | KERT 方案 A（全量 Java）、方案 B（全量 Python）、方案 C（Python Core + Spring AI Alibaba Skill Runtime） |
| 目标决策 | 是否批准方案 C 进入 KERT 正式架构基线与后续生产演进 |
...
GITS_INTEGRATION_PATH=HTTP_TO_KERT_PUBLIC_API_ONLY
...
> 本报告允许 Owner 批准“修改后的方案 C 作为目标架构方向，并开展下一轮准入验证”；不表示 Java Skill Runtime 已达到生产标准，也不表示 KERT、GITS UAT 或生产发布门禁已经通过。
...
1. **方案 C 比方案 A 更合理。** KERT 已有 Python 数据/知识核心，全量改写为 Java 缺少必要收益证据，且会引入大规模重写、回归和数据能力替换风险。当前材料不支持批准方案 A。
2. **方案 C 只有在修改后才可能优于方案 B。** 方案 B 仍是当前更简单、更可控的实施基线；方案 C 的长期优势取决于 Java Skill Runtime 能否通过内部契约、沙箱、版本治理、可靠性和运维准入。如果准入失败，KERT 应继续按方案 B 演进，而不是阻塞生产加固。
3. **当前方案 C 的边界尚不清晰。** 材料同时允许 Java Runtime 作为 KERT 内部组件、独立服务，甚至部署到 GITS 侧并由 GITS 直连。这会造成外部入口、Skill 权威、审计、幂等、身份和故障责任分裂，必须在基线前消除。
4. **推荐采用 C′（修改后的混合架构）：** Python Core 是 KERT 唯一外部入口和控制面；Java Runtime 是 KERT 产品内部、无权威状态、可替换的受控执行器；GITS 只通过 KERT 公共 HTTP/OpenAPI 调用，不得直连 Java Runtime。
...
8. **GITS A+B 不应等待 Java Runtime。** B（撤销 Mock/H2 伪成功、fail-closed）先落地，A（恢复 GITS→KERT 真实 HTTP）紧接执行；首轮真实 E2E 直接对接 Python Core。Java Runtime 是后续内部实现变化，不应改变 GITS 公共契约。
...
| GITS 联动是否顺畅 | 只在 GITS→KERT Python Core 单一 HTTP 路径下顺畅；直连 Java Runtime 不批准 |
...
1. `docs/architecture/KERT_SPRING_AI_ALIBABA_VS_PYTHON_NATIVE_COMPARISON_V1.0.md`；
2. `docs/architecture/KERT_ADAPTER_FIRST_TECH_STACK_V1.1.md`；
3. `docs/architecture/KERT_SPRING_AI_ALIBABA_SKILL_RUNTIME_POC_V1.0.md`；
4. `poc/spring-ai-alibaba-skill-runtime/POC_RESULT.md`；
5. `docs/dd/KERT_independent_architecture_review_2026-08-26_V1.0.md`；
6. `docs/architecture/KERT_PRODUCTION_EVOLUTION_PLAN_V2_CANDIDATE.md`；
7. `docs/integration/GITS_KERT_A_PLUS_B_HANDOFF.md`；
...
| 对象 | 状态 | 依据与限制 |
|---|---|---|
| Python Core 已有数据/知识层 | `CURRENT_DOCUMENTED / PARTIAL_VERIFICATION` | Phase 0 状态与既有独立评审明确；本包未包含完整 KERT Python 源码和受控 commit |
...
3. `KERT_PHASE0_EVIDENCE_MANIFEST.json` 把若干测试标记为 `verified=true`，状态基线却称原始报告 `DOCUMENTED_BUT_NOT_VERIFIED`；提供的测试日志只有点号进度，没有命令、环境、用例名和退出码上下文。
4. 生产演进计划 V2 没有纳入 Java Runtime；核心对比文档又新增一套 Phase 0-5 顺序，且将 GITS A+B 推迟到 Phase 4，与既有独立评审“尽早 A+B”的路线冲突。
5. 独立服务边界文档仍把 `kert-server` 定义为 HTTP API、Skill 执行和知识服务的单一组件；方案 C 新增 Java Runtime 后，组件模型、部署包和责任边界尚未更新。
...
当前 POC 证明的是“框架具有 Skills Hook 和动态工具披露能力”，不是“KERT 已得到生产级 Skill Runtime”。以下责任仍由 KERT 自己建设：
...
```mermaid
flowchart TB
    G["GITS / 其他客户端"] -->|"公共 HTTP + OpenAPI"| P["KERT Python Core\n唯一入口与控制面"]
...
| 幂等、Job、Gate、审计 | **权威并唯一写 Runtime Store** | 不直接写 SQLite；返回回执 | 返回进程级回执 | 不得伪造 KERT 结果 |
| 五层知识资产、数据源、证据 | **权威** | 只通过受控 Context/Source API 读取 | 不直接访问 Core | 不直读文件 |
| Skill 包审核、版本、激活和撤销 | **权威控制记录** | 只加载已激活的 ID/version/hash | 只执行指定版本 | 不管理 KERT Skill |
...
8. 把 Java Runtime 部署到 GITS 进程内并继续称其为 KERT 独立服务能力。
...
- `TraceContext`：W3C trace context + KERT requestId；
...
必须把 Java Runtime降格为“可替换内部执行器”，而不是第二个 KERT 控制面。所有外部请求必须由 Python Core 建立身份、幂等、Job、Policy 和 ExecutionPlan 后再调 Java。
...
PyArrow、DuckDB、Kùzu、Pydantic/FastAPI 以及 `KnowledgeSource` typed capability 与现有 KERT 方向一致。材料没有显示把这些能力重写成 Java 能取得可验证收益。图、Parquet、RAG、受控 HTTP/JDBC 等适配器应继续放在 Python Core；Java Runtime 通过受控 Source API 或执行上下文获取必要信息，不直接连接客户数据源。

这也意味着 Java Runtime 不应拥有第二套 Source 凭据、SQL 模板、数据脱敏和证据来源逻辑。

### 5.5 GITS 联动

**结论：公共路径必须是 GITS→KERT Python Core HTTP。**

GITS 与 Java Runtime 同为 Java 并不足以支持“直连”。直连会：

- 绕过 KERT 的公共 OpenAPI 和合同 hash；
- 绕过 Python Core 的幂等、Job、Gate、审计、成本和证据装配；
- 形成两套超时、错误和 fail-closed 语义；
- 使 KERT-owned 能力再次嵌入 GITS，破坏独立服务边界；
- 让 A+B UAT 无法证明实际经过完整 KERT 控制面。

GITS A+B 的正确顺序不变：

1. B：先删除/禁用 Mock、H2、fallback 伪成功，KERT 不可用时明确空态；
2. A：恢复 GITS→KERT 公共 HTTP Adapter；
3. 对 Python Core 完成 R1、供应链、assemblyTrace、SP-20、SP-21、Gate 真实 E2E；
4. Java Runtime 后续接入属于 KERT 内部实现演进，GITS 契约和调用地址保持不变；
...
- 一个 KERT 安装包/Compose 项目，而不是两套独立产品；
...
方案 B 是有效替代路线：如果团队无法维护双栈，或 Java Runtime 准入 POC 未通过，继续用 Python Core + OS Sandbox + 受控 Tool ABI 不会破坏 KERT 总体架构。
...
| `C-B04` | POC 未完成 KERT 集成、真实 Tool receipt、生产热更新/回滚和故障恢复 | 严重：框架能力无法转化为产品能力 | 执行 POC-2 准入矩阵 | 原始日志、自动化测试、受控 commit、E2E |
...
3. 将 POC API 路径放入明确的内部 namespace，例如 `/internal/v1/runtime/*`，禁止与 KERT 公共 API 混用。
...
9. 清理交接材料中残留的外部开发运行环境产品依赖与命名，保持 KERT 独立服务边界。
...
Gate C-RUNTIME-01：Java Runtime POC-2 准入
  ↓ PASS
Phase 2：以 KERT 内部无状态执行器方式接入 Java Runtime
  ↓
...
任何一项未通过时，Java Runtime 保持实验组件，KERT 继续按 Python-only 路线运行。
...
不建议选择。只有在 Owner 明确决定 KERT 全产品统一 Java、接受 Python 数据/知识层重写，并提供迁移成本、性能收益、功能等价和回滚证据后，才应重新评估。当前材料没有这些前提。
...
- 不批准全量 Java 重写；
- 不批准 GITS 直连或内嵌 KERT Java Skill Runtime；
- 不批准当前 POC 直接进入生产；
...
JAVA_RUNTIME_DEPLOYMENT=KERT_OWNED_SEPARATE_PROCESS
PUBLIC_API_OWNER=KERT_PYTHON_CORE
RUNTIME_STATE_OWNER=KERT_PYTHON_CORE
...
| ID | 待补充材料 | 用途 | 建议责任方 |
|---|---|---|---|
| `C-ME01` | KERT Python Core 受控源码 commit、manifest | 验证现有能力和真实边界 | KERT Tech Lead |
...
| `C-ME14` | GITS→KERT 最终 HEAD HTTP trace、合同 hash、A+B UAT 原始报告 | 确认联动 | GITS/KERT/UAT Owner |
...
方案 C 的价值成立：它避免重写 KERT 已有 Python 数据/知识层，同时利用 Spring AI Alibaba 的 Skill Hook 和工具渐进披露能力。但当前方案把“框架能力可用”推导为“生产 Skill Runtime 成熟”，证据跨度过大；部署形态、控制面、内部契约、沙箱和 GITS 路径也尚未收敛。
