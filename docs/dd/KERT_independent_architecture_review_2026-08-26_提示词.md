你现在担任 **KERT Tech Lead / 架构整改负责人**。你的任务不是重新做一次泛化评审，而是依据已经完成的独立架构评审，形成并执行一套受控、可追溯、可供 Owner 离线审批的 **Phase 0 架构与基线整改包**。
# 强制前置输入

本次整改的首要权威输入是：
~/dev/deepseek_harness/data_knowledge_ws/kert/docs/dd/KERT_independent_architecture_review_2026-08-26_V1.0.md

如果找不到或无法读取该报告，必须停止实质整改并输出：
RESULT=BLOCKED
BLOCKED_REASON=缺少独立架构评审报告原件
MISSING_INPUT=KERT_independent_architecture_review_2026-08-26_V1.0.md
...
外部智能体、GITS 或其他系统只能作为 KERT 的 API 客户端，不能成为 KERT 启动、配置、运行、密钥管理、Skill 加载或运维的必要依赖。

本次工作目标是：

1. 收敛 KERT 当前相互冲突的架构、状态和契约文档；
2. 明确独立服务端产品边界；
...
* 检查 KERT 工作区、Git 状态、代码、文档、契约和测试；
* 在 KERT 项目内新增或修改 Phase 0 所需的架构、契约、ADR、治理、追溯和测试设计文件；
...
```text
/home/szf/dev/deepseek_harness/data_knowledge_ws/kert-交接评审包-2026-08-26.tar.gz
/home/szf/dev/deepseek_harness/data_knowledge_ws/kert-交接评审包-2026-08-26/
```
...
5. 不得覆盖与本任务无关的未提交修改；
6. 如果找不到 KERT 源代码，只能完成材料允许的设计整改，不得宣称完成实现验证。

建议阅读顺序：

1. 独立架构评审报告；
2. `HANDOVER.md` / `HANDOVER.txt`；
3. `kert/docs/handover-review-2026-08-26.md`；
4. `kert/docs/production-evolution-plan.md`；
5. `kert/docs/architecture.md`；
6. `kert/ADR.md`；
7. `kert/docs/skill-execute-api-contract.md`；
8. `kert/docs/skill-execute-api-contract-v1.4.md`；
9. `kert/docs/v14-joint-debugging-plan.md`；
10. `kert/README.md`；
11. 详细需求与详细设计文档；
...
> 在一台干净主机上，不安装、不启动任何 Agent Harness 或类似平台，KERT 仍能完成安装、配置、启动、健康检查、Skill 查询、Skill 执行、任务恢复、证据读取和受控工具调用。
...
为 KERT 建立 OpenAPI 3.1 和 JSON Schema 契约唯一权威源。
...
* 不得用本地 Mock、H2、fallback 数据冒充 KERT 返回；
* KERT 未配置、不可用、鉴权失败或契约错误时，必须显示明确的 fail-closed 空状态；
* 不得让业务层误认为 KERT 调用成功；
* 必须保留错误类型和 correlation ID。

### A：随后恢复真实 HTTP 对接

* 建立 GITS → KERT HTTP adapter；
* 配置名使用 `KERT_BASE_URL`、`KERT_API_KEY`；
* 不使用任何外部智能体平台的命名；
* 建立契约测试；
* 建立真实 E2E；
* 验证 assemblyTrace、供应链图谱、SP-20、SP-21 和 Gate；
* 验证超时、重试、鉴权失败、契约错误和 KERT 不可用。
...
```text
docs/governance/KERT_PHASE0_DECISION_RECORD.md
docs/governance/KERT_STATUS_BASELINE_CANDIDATE.yaml
docs/governance/KERT_DOCUMENT_CONFLICT_REGISTER.md
docs/governance/KERT_SUPERSESSION_MAP.md
docs/governance/KERT_BLOCKER_CLOSURE_MATRIX.md
docs/governance/KERT_TRACEABILITY_MATRIX.md

docs/architecture/KERT_INDEPENDENT_SERVER_BOUNDARY_V1.0.md
docs/architecture/KERT_RUNTIME_CONTROL_PLANE_V1.0.md
docs/architecture/KERT_PRODUCTION_NFR_BASELINE_V1.0.md
docs/architecture/KERT_PRODUCTION_EVOLUTION_PLAN_V2_CANDIDATE.md
...
docs/contracts/openapi/kert-openapi-v2.yaml
...
docs/integration/GITS_KERT_A_PLUS_B_HANDOFF.md
docs/testing/KERT_PHASE1_ACCEPTANCE_TEST_PLAN.md
evidence/phase0/KERT_PHASE0_EVIDENCE_MANIFEST.json
```
...
| B-06 | GITS 当前未实际调用 KERT             | 形成 A+B 交接；保持 UAT_PASS=NO  |
...
WORKSTREAM=KERT-PHASE0-ARCH-REMEDIATION-01
