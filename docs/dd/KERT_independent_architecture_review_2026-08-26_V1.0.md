# KERT 知识工程运行态独立架构评审报告

| 控制项 | 内容 |
|---|---|
| 文档编号 | `KERT-ARCH-REVIEW-2026-08-26-001` |
| 文档版本 | `V1.0` |
| 评审日期 | `2026-08-26` |
| 评审角色 | Independent Enterprise AI Platform Architecture Reviewer / QA Gate |
| 评审对象 | KERT 交接包、当前架构、v1.3/v1.4 契约、GITS 联调/UAT 结论、生产演进 Phase 1-5 |
| 强制产品边界 | KERT 是可独立运行的服务端软件，不依赖任何 Agent Harness 或其他开发运行平台 |
...
1. **KERT 的总体技术方向成立。** 五层工作区、`03_core` 权威资产、`04_serve` 可重建投影、知识接入—审核—发布—查询闭环，以及 Skill 执行、规则、图谱、证据溯源，足以支撑“知识工程运行态”的原型定位。
...
7. **`UAT_PASS=NO` 结论正确。** 即使 KERT 自测全部通过，只要当前 GITS 分支没有真实调用 KERT，就不能通过端到端 UAT。材料描述的根因“GITS 当前分支缺少 Skill HTTP 适配器和服务地址配置”逻辑成立，但评审包未包含 GITS 源码、分支差异和 UAT 原始日志，因此根因的独立复核状态为“待补充”。
8. **Owner 应选择 A+B。** 先实施 B：撤销 KERT 所有权能力的本地 Mock/H2 拼装并 fail-closed；同时实施 A：在独立合同 Loop 中恢复真实 GITS→KERT HTTP 适配器并完成端到端 UAT。配置名应改为产品中性的 `KERT_BASE_URL`，不再沿用外部运行环境命名。
...
- `HANDOVER.md`；
- `kert/docs/handover-review-2026-08-26.md`；
- `kert/docs/production-evolution-plan.md`；
- `kert/docs/architecture.md`；
- `kert/ADR.md`；
- `kert/docs/skill-execute-api-contract.md`；
- `kert/docs/skill-execute-api-contract-v1.4.md`；
- `kert/docs/v14-joint-debugging-plan.md`；
- `kert/README.md`；
...
- `kert-server`、`kert-worker`、`kert-admin` 可在干净主机/容器独立安装启动；
- Skill 资产随 KERT 发布包自带并有版本/签名；
- 密钥只从 KERT 自身配置/Secret Provider 获取；
- GITS 只是 API Consumer；可视化插件只是可选客户端；
- 移除产品运行对外部 session、插件注册表、用户目录凭据文件的依赖；
- GITS 配置改为 `KERT_BASE_URL`、`KERT_API_KEY` 等产品中性名称。
...
| 通用 Agent 化 | `MISSING` | 也不是 KERT 必须承担的自主规划职责；应提供受控工具/技能 API，而非内置无限自主 Agent |
...
| 放行控制 | KERT 信任 request 中 passed gates，不能单独阻止伪造放行 |
...
6. 建立 consumer-driven contract test，KERT 与 GITS 在同一版本矩阵验证；
...
UAT 判断依据是端到端业务链是否真实经过 KERT，而不是 KERT 自身测试是否绿色。材料明确称当前 GITS 页面使用本地知识快照、Mock LLM、H2 规则或空壳，不会调用 KERT，因此不能通过。
...
| B：先 fail-closed 空态 | 立即停止 Mock/H2 冒充 KERT 结果，保护事实和 UAT 真实性 | 功能仍不可用，只是正确失败 | 否 |
| A+B | 同时保证“真调用”和“失败不伪造” | 需要一次受控合同/适配器 Loop 和完整 E2E | **是，推荐** |

#### 推荐执行顺序

1. **B 先落地**：KERT-owned capability 在未配置、不可达、超时、认证失败、合同错误时显示明确空态；不得本地拼装同名结果；
2. **A 紧接实施**：恢复 GITS `SkillExecutionPort/ProposalPort/InteractionMemoryPort` 的 KERT HTTP Adapter；
3. 配置统一为 `KERT_BASE_URL`，不要使用 DHCP 地址作为生产配置；
4. consumer contract tests 覆盖 v1.3/v1.4；
5. 最终 HEAD 上执行真实 E2E：R1、供应链图谱、assemblyTrace、SP-20 async、SP-21、Gate；
6. 注入 KERT 不可达/超时/404/422/PARTIAL/Job FAILED，证明 fail-closed；
7. 形成 GITS 与 KERT 两侧同一 trace/requestId 的证据包后再重新 UAT。
...
| `B-03` | 独立服务边界未闭合，运行说明仍依赖外部开发运行环境的配置、凭据和 Skill 同步 | KERT 无法作为独立产品安装、升级、审计和支持 | 高 | 严重 | Phase 0 清理产品依赖，独立发行、配置、Secret、Skill 包和服务名 | 干净主机/容器离线安装启动与功能验证 |
| `B-04` | v1.3/v1.4 没有机器合同 SSOT，ContextPackage 缺失，文档内存在字段/端口/版本矛盾 | GITS DTO 漂移、误解析、无法证明兼容 | 高 | 严重 | OpenAPI/JSON Schema 基线、合同版本、consumer tests、冲突清零 | KERT/GITS 双方同一合同 hash 与测试报告 |
| `B-05` | 评审包无源码/commit、原始测试/扫描/运行证据 | 无法独立确认“已实现、137 全绿、服务健康” | 中 | 严重 | 补受控实现审计包并由独立 QA 复跑 | commit、manifest、测试、覆盖率、安全、运行日志 |
| `B-06` | GITS 当前未真实调用 KERT | 端到端业务链不存在，UAT 不能通过 | 高 | 严重 | 执行 A+B，最终 HEAD 真实 E2E | 两侧 trace、HTTP 证据、UI 结果、故障空态、Owner UAT |
...
| `M-15` | SP-20 信任 GITS 传入 gateState，可生成可放行对客版 | 伪造审批状态导致对客风险 | 中/高 | 使用认证服务身份 + 权威 gate assertion/token/API；KERT 不信任自由字段 |
| `M-16` | KERT 知识与 GITS 业务事实冲突优先级写法不清 | 旧知识可能覆盖最新业务事实 | 中/高 | 建立 Source Authority/时态/冲突合同，不用“KERT 总是优先” |
...
| 发现范围 | 责任工作流 | 复核方 |
|---|---|---|
| `B-01`、`M-12` | KERT Security/DevOps | Independent Security QA |
| `B-02`、`M-03`—`M-05` | KERT Runtime Tech Lead | Reliability QA |
| `B-03`、`M-01`—`M-02` | KERT Architecture/Baseline Owner | Architecture Review |
| `B-04`、`M-15`—`M-16` | KERT/GITS Contract Owners | Consumer Contract QA |
...
1. 批准 KERT 的产品定位：**独立运行、单机优先、文件知识权威源、SQLite 运行控制状态**；
...
- 不批准在无真实 GITS→KERT 调用时把 UAT 改为 PASS；
...
|---|---|---|---|
| `ME-01` | KERT 源码快照、commit、文件 manifest/hash | 独立验证实现 | KERT Tech Lead |
...
| `ME-09` | OpenAPI/JSON Schema 当前文件（若存在） | 机器契约审查 | KERT/GITS Contract Owner |
...
- 最终 GITS HEAD 真实调用最终 KERT HEAD；
- R1、供应链、SP-20、SP-21、Gate/trace 全链证据；
- KERT 不可达/超时/认证失败/PARTIAL 时 GITS 明确 fail-closed；
- 无 H2/Mock 冒充 KERT-owned 结果；
...
KERT 已经形成一个结构扎实的知识工程原型核心：文件化权威资产、不可变 Core、可重建 Serve、知识发布门禁、证据溯源以及面向银行场景的 Skill/规则/图谱链路，这些都值得保留。其生产演进应采用**增量加固**，而非重写。
