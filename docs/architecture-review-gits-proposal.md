# KERT 端架构评审：GITS 服务建议书与交互记忆集成（SP-20/SP-21）

> 评审人：KERT 端架构师 ｜ 日期：2026-08-23
> 评审对象：GITS `docs/architecture/GITS-KERT-SERVICE-PROPOSAL-INTEGRATION-DESIGN.md` + 附录 A/B/C/D
> 立场：**知识在 KERT，业务在 GITS**（延续 v1.3 数据所有权原则），SP-20/SP-21 作为 KERT 技能落地，
> 闸门/记忆生命周期/业务版本由 GITS 编排，KERT 提供知识装配、生成、标签、规则与过滤引擎。
...
（闸门状态机、交互记忆生命周期、业务版本管理）划给了 KERT**，这与 KERT 的平台定位（文件目录型
**知识服务**平台，非业务应用）冲突，也与 v1.3 已确立的"知识在 KERT、业务在 GITS"原则相悖。

**整体最佳方案一句话**：KERT 承接 **SP-20（建议书知识装配+逐章生成+事实标签+引用+规则校验+确定性
双版本过滤）** 与 **SP-21（记忆抽取）**，输出结构化 ServiceResult；GITS 承接 **闸门状态机（G0-G5）、
交互记忆存储与生命周期、业务版本记录、审批流**；KERT 只通过知识/技能接口被调用，不成为第二个业务系统。

### 1.1 职责边界总表

| 能力 | 归属 | 理由 |
|---|---|---|
| 行业/财务/模板/规则知识资产 | **KERT** | 本就是知识资产（03_core 可版本化） |
| SP-20 逐章生成 + 事实标签 + 引用索引 | **KERT** | 技能/生成是 KERT 职责；标签与生成同源才能自洽 |
| 6 条 BLOCKING 规则校验（CITATION/DUAL_VERSION/…） | **KERT（RULE 资产 + evaluate_rule）** | 规则是知识，校验结果随结果返回 |
| 内部版/对客版**过滤执行**（确定性、可审计） | **KERT（过滤引擎，无 LLM）** | 标签在 KERT 生成，过滤逻辑就近、可复现 |
| 对客版**发布权**（G1+G2+G3 通过后才放行） | **GITS（闸门）** | 审批权在业务端 |
| 闸门状态机 G0-G5 | **GITS** | 业务编排+人工审批；KERT 90_control 是平台治理闸门，**不是**业务闸门，不可混用 |
| 交互记忆**抽取**（SP-21） | **KERT** | 抽取=知识化，LLM 技能职责 |
| 交互记忆**存储与生命周期**（候选/确认/过期/取代+置信度衰减） | **GITS** | 操作性状态，非知识资产；v1.3 已定客户主档不写知识，反之亦然 |
| 建议书**业务版本**（v1.0/v1.1/v2.0） | **GITS（ProposalVersion 记录）+ KERT（知识快照 release_version）双轨** | 业务版本是 GITS 记录；KERT 每次生成落一个不可变知识快照供引用 |
...
GITS 建议的 `01_raw/{customerId}/proposal/...` 按客户+业务对象建目录，**与 KERT 域/版本模型冲突**
（KERT 是 `03_core/<domain>/version=<release>/` 的域资产模型，不是客户档案柜）。
...
> GITS 的 v1.0/v1.1 业务版本号 ↔ KERT release_version 由引用索引（runId/citation id）关联，互不覆盖。
...
### D3：双版本生成逻辑 —— **KERT 生成内部版 + 确定性过滤出对客版，GITS 控制放行时机**

- 事实标签在生成时产生，过滤规则与标签同源才能自洽、可复现、可审计 → **过滤执行放 KERT**（无 LLM，
  纯规则，输出 filtering-report.json）。
- **GITS 保留发布权**：对客版在 `data.content.customerVersion` 中**已生成但带 `releaseBlockedUntil: ["G1","G2","G3"]`**，
  GITS 闸门通过后才向前端展示/导出。避免 KERT 越权审批。

### D4：交互记忆存储 —— **放 GITS，KERT 只做抽取（SP-21）**

- 记忆有 CANDIDATE→CONFIRMED→EXPIRED/SUPERSEDED 生命周期 + 置信度衰减，是**操作性业务状态**，
  放 KERT 03_core 会污染"不可变知识资产"语义（记忆在变、知识不可变）。
- **裁定**：GITS 领域模块（H2）存储与生命周期；KERT SP-21 从纪要抽取候选（输出结构化 statements 风格
  候选 + 与已有记忆的比对建议），GITS 调 `InteractionMemoryPort` 持久化/确认。
- 交互记忆进入建议书：GITS 组装进 ContextPackage.interactionMemory（已有设计），KERT 不持有记忆库。

### D5：闸门状态机 —— **GITS 唯一权威，KERT 提供闸门清单知识与就绪度建议**

- 必须明确：KERT 90_control 的 G0-G5 是**平台知识资产门禁**（ingest→…→projection），与业务 G0-G5
  （证据→核验→专家→审批→对客→复盘）**同名异义**，混用会造成审计混乱。
- **裁定**：业务闸门状态机 = GITS（ProposalPort.advanceGate），人工审批在 GITS；
  KERT 在 SP-20 输出 `gateRecommendations`（就绪度评估 + checklist），checklist 本身作为
  **KERT RULE/知识资产**（每闸门"必须完成/禁止事项"来自交接文档，落 `03_core/proposal-gates/`）；
  GITS 可（可选）把闸门决策**镜像**到 KERT 审计端点，但权威在 GITS。
...
### D8：ContextPackage 大小 —— **采纳 50KB 上限，KERT 侧相关性裁剪**

- 50KB（压缩后）≈ 25-30K tokens，DeepSeek 上下文可容纳。
- KERT 侧裁剪策略：行业框架只注入目标章节相关段；交互记忆按置信度/时效 Top-N（≤20 条）；
  财务摘要只取最近 3 年 + 关键比率；裁剪在 `limitations` 中显式声明（不静默降级）。

---

## 3. SP-20 在 KERT 的实现设计
...
- **规则校验**：6 条 BLOCKING 规则（CITATION_REQUIRED / NO_UNDISCLOSED_DEGRADATION /
  ...
  落为 **KERT RULE 资产**（复用 `evaluate_rule` DSL），生成后自动校验，违规以 `data.ruleViolations`
...
- SP-20 是**组合型业务技能**：需要企业内数据（财务/授信/交易）+ 公开数据 + 交互历史 + 交互记忆，
  这些大多**不在 KERT 库内**（财务/授信在 GITS 业务侧）。因此 SP-20 的 `request.context`（ContextPackage）
  是**必要例外**：GITS 组装上下文传入，KERT 负责**知识化 + 生成 + 标签 + 校验**。
- 边界：ContextPackage 里**银行内数据**（授信/交易）只用于生成，**不落 KERT 权威库**
  （除证据登记到 01_raw 批次），延续"KERT 不持有业务主档"原则；KERT 库内的行业/财务框架与交互记忆
  （若有）**优先于** context 中同名数据（知识在 KERT）。
...
- **KERT 不存记忆**：candidateMemories 交 GITS `InteractionMemoryPort.confirmCandidate` 持久化。
...
| 两套"G0-G5"（平台门禁 vs 业务闸门）语义混淆 | 命名分离：KERT `GATE-PLATFORM-*`，GITS `GATE-BIZ-*`；文档明示 |
| ContextPackage 含银行内数据越界 | 只生成不落权威库；证据登记 01_raw 批次；KERT 库知识优先于 context 同名数据 |
...
> 状态更新 2026-08-23：Phase 1 ✅ 完成；Phase 2 完成 7/8/9 的 KERT 侧（7-8 已落地，9 待 GITS 联调）。
...
KERT 把 SP-20/SP-21 做成**一等技能**（知识装配 + 逐章生成 + 事实标签 + 规则校验 + 确定性双版本过滤 +
