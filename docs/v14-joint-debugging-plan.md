# KERT × GITS v1.4 联调准备（环境 + 用例清单 + 实测结果）

> 日期：2026-08-23 ｜ 状态：**KERT 侧就绪，可联调**（以下用例全部在 8106 真实通过）
...
| 项 | 值 |
|---|---|
| KERT 服务 | `http://127.0.0.1:8106`（同机）／ `http://192.168.31.220:8106`（WiFi LAN） |
| **地址变更警示** | 旧文档中的 `172.22.90.134` **已失效**（本机 IP 变更）。GITS 启动时设 `DSH_BASE_URL=http://192.168.31.220:8106`（或同机 `127.0.0.1:8106`） |
| 服务方式 | systemd 用户服务 `kert-skill`（Restart=always），最新代码（SP-20/SP-21/闸门端点）已加载 |
...
| 认证 | 无（演示环境网络层控制）；如启用鉴权由 GITS 网关加 X-API-KEY，KERT 透传不校验 |
