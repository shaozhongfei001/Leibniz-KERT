# KERT 适配器优先技术栈 V1.1（候选）

> 状态：CANDIDATE
> 日期：2026-08-26
> 替代：`KERT_OPEN_SOURCE_TECH_STACK_V1.0.md`（V1.0 范围过大，保留为历史）
> 原则：KERT 只做能力接口 + 轻量适配器 + 本地默认实现，不负责部署外部重型系统。

## 1. 边界声明

KERT 不引入/不部署以下系统作为自身组件：
...
以上系统可以**已经存在**于客户环境，KERT 只提供访问适配器。

## 2. 核心运行栈

| 层 | 选型 | 理由 |
|---|---|---|
| 语言 | Python 3.11 | 与现有 KERT 一致，避免重写 |
...
| 热更新生效 | `kert-admin skill activate <skillId> <version>` | 原子切换 CURRENT 指针 |
...
| 与现有 KERT | 需引入 Java 服务或重写 Skill 运行层 | 与现有代码一致 |
...
2. 是否采用取决于产品架构决策：
   - 如果 KERT 保持 Python 单体，则继续用 Python 原生轻量方案；
   - 如果允许引入独立的 Java Skill 运行态服务，Spring AI Alibaba 可以作为该服务的基座，KERT Python 核心只负责知识/数据服务；
...
1. 是否接受“适配器优先、不部署外部系统”作为 KERT 技术栈原则？
