...
> 实现细节见 `KERT_RUNTIME_HARDENING_M2P1.md`。

### 2.1 TLS 终止位置

**TLS 由外层反向代理（Nginx/网关）终止，KERT 应用层不实现 TLS。**
...
Python Core 的 `KERT_BIND_HOST` 参与生产 profile 校验：
...
- 代理层大小限制不小于应用层 `KERT_MAX_REQUEST_BYTES`，避免应用层 413 被代理层提前遮蔽
...
> M2.5 已落地，实现细节见 `KERT_OBSERVABILITY_M2P3.md`。

- 统一 JSON 日志字段：requestId, traceId, tenantId, skillId
  （`KERT_STRUCTURED_LOGS=true` 启用，生产默认开启）
...
`KERT_METRICS_REQUIRE_ADMIN` 单一决定）。

### 6.2 采集侧要求

- **`/metrics` 需保护**：指标含队列深度、DB schema 版本等内部信息。
  生产须设 `KERT_METRICS_REQUIRE_ADMIN=true`，或由网络层限制采集来源。
...
> M2.6 已落地，实现见 `src/kert/infrastructure/backup.py`，
> 运维入口 `scripts/kert_ops.py`。演练报告见 `evidence/m2-p5/`。
...
```bash
# 备份（目标必须在工作区之外）
python scripts/kert_ops.py backup --workspace ./workspace --dest /var/backups/kert

# 校验完整性（逐文件 sha256）
python scripts/kert_ops.py verify --backup /var/backups/kert/backup-<ts>
```
...
python scripts/kert_ops.py restore \
    --backup /var/backups/kert/backup-<ts> --target /srv/kert-restored
```
...
python scripts/kert_ops.py manifest --out /srv/releases/RELEASE_MANIFEST.json

# 升级/回滚前比对差异
python scripts/kert_ops.py compare --left old.json --right new.json
```
