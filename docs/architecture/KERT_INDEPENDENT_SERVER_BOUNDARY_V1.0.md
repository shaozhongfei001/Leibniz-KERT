...
明确 KERT 作为独立安装、配置、启动、运行、升级和审计的知识工程服务端软件的产品边界。
外部智能体、GITS 或其他系统只能是 API 客户端，不能成为 KERT 启动、配置、运行、密钥管理、Skill 加载或运维的必要依赖。

## 2. 产品组件

| 组件 | 名称 | 职责 |
|---|---|---|
| 服务端 | `kert-server` | HTTP API、Skill 执行、知识服务 |
| 异步 Worker | `kert-worker` | 持久化 Job 消费、重试、租约、恢复 |
| 管理 CLI | `kert-admin` | 安装、配置、密钥注入、Skill 安装、状态查看、备份恢复 |
| 配置目录 | `KERT_CONFIG_DIR` | 服务配置、密钥引用、Skill 资产注册 |
...
- 配置来源：`KERT_CONFIG_DIR/kert.yaml` + 环境变量
- 端口、监听、工作区、Runtime DB、日志级别均为配置项
- 禁止在合同/文档中硬编码环境 IP

### 4.3 密钥管理

- 通过 `KERT_SECRET_PROVIDER` 抽象注入
...
- Skill 资产随 KERT 发布包自带
- 每个 Skill 包包含 SKILL.md、schema、版本、哈希
- 安装/升级通过 `kert-admin skill install <package>` 受控执行
- 不依赖外部插件注册表
...
- GITS 配置：`KERT_BASE_URL`、`KERT_API_KEY`
- 不使用 `DSH_BASE_URL` 作为产品配置名
- 客户端必须支持认证、超时、错误空态

## 6. 独立性验收测试

在一台干净主机上，不安装、不启动任何 Agent Harness 或类似平台，KERT 必须能完成：
...
- 无 `kert-admin`、Docker、CI → Phase 1 补齐
