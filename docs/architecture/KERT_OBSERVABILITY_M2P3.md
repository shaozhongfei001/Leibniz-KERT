# KERT 可观测性说明（M2-P3 / M2.5）

对应任务包：`M2-P3`，范围 `M2.5 可观测性`。
依据：`ADR-015`（单节点生产 profile）、规格 `§9.20`（作业日志契约）、
`KERT_HYBRID_DEPLOYMENT_AND_OPERATIONS_V1.0_CANDIDATE.md` §6。

> **非声明**：本文档不代表 KERT 已生产就绪，不代表安全审计已完成，
...
| 环境变量 | 默认 | 说明 |
|---|---|---|
| `KERT_STRUCTURED_LOGS` | **prod=true / dev=false** | 是否切为单行 JSON 输出 stdout |
| `KERT_LOG_LEVEL` | `INFO` | 日志级别 |
| `KERT_SERVICE_NAME` | `kert-python-core` | 写入每条日志的服务标识 |
...
```json
{"status": "alive", "service": "kert-python-core",
...
| Runtime Store 可连接 | 取决于 `KERT_READINESS_REQUIRE_STORE`（默认 true） |
...
| 环境变量 | 默认 | 说明 |
|---|---|---|
| `KERT_METRICS_ENABLED` | `true` | 关闭后返回 404 |
| `KERT_METRICS_REQUIRE_ADMIN` | `false` | 是否要求 admin 作用域 |
...
| 指标 | 类型 | 标签 | 说明 |
|---|---|---|---|
| `kert_http_requests_total` | counter | method, path, status | HTTP 请求总数 |
| `kert_http_request_duration_seconds` | histogram | method, path | 请求耗时（11 个分桶，5ms~10s） |
| `kert_http_client_errors_total` | counter | method, path, status | 4xx 响应数 |
| `kert_http_server_errors_total` | counter | method, path, status | 5xx 响应数 |
| `kert_http_errors_total` | counter | method, path | 未捕获异常数 |
| `kert_readiness` | gauge | — | 1=就绪，0=未就绪 |
| `kert_job_queue_claimable` | gauge | — | 可领取 Job 数 |
| `kert_job_queue_dead_letter` | gauge | — | dead-letter Job 数 |
| `kert_job_queue_expired_leases` | gauge | — | 待回收 Job 数 |
| `kert_job_status_count` | gauge | status | 各状态 Job 数 |
| `kert_process_uptime_seconds` | gauge | — | 进程运行时长 |
| `kert_build_info` | gauge | version, profile | 构建信息 |
| `kert_evidence_audit_errors_total` | counter | — | evidence 审计写入失败数 |
| `kert_metrics_collection_errors_total` | counter | — | 指标采集失败数 |
...
```
kert_http_requests_total{method="GET",path="/v1/evidence/{object_id}",status="200"}
```
...
| 环境变量 | 默认 | 说明 |
|---|---|---|
| `KERT_TRACING_ENABLED` | `true` | 是否记录 span |
| `KERT_TRACE_SAMPLE_RATIO` | `1.0` | 采样率 0.0~1.0 |
| `KERT_OTEL_ENABLED` | `false` | 是否尝试桥接 OpenTelemetry SDK |
...
需要真实导出时设 `KERT_OTEL_ENABLED=true` 并安装 OTel SDK；
...
export KERT_API_KEYS="svc:<secret>:read|execute,ops:<secret>:read|execute|admin"
export KERT_RATE_LIMIT_ENABLED=true
export KERT_RUNTIME_STORE_ENABLED=true
export KERT_METRICS_REQUIRE_ADMIN=true
...
```yaml
scrape_configs:
  - job_name: kert-python-core
    metrics_path: /metrics
    static_configs:
      - targets: ["127.0.0.1:8106"]
    # metrics_require_admin=true 时需携带密钥
    authorization:
      credentials_file: /etc/prometheus/kert-admin-key
```
...
| 无 OTLP 导出器配置 | 内置追踪不做网络导出；需真实导出时装 OTel SDK 并设 `KERT_OTEL_ENABLED=true` | 后续 |
