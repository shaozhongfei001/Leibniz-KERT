...
> 非声明：本文档不代表 KERT 已生产就绪，不代表安全审计已完成，不代表 GITS UAT 已通过。
...
| 变量 | 默认 | 说明 |
|---|---|---|
| `KERT_PROFILE` | `dev` | `dev` / `prod` |
| `KERT_BIND_HOST` | `127.0.0.1`（配置层默认） | 监听地址，纳入生产校验 |
| `KERT_AUTH_ENABLED` | 有密钥时为 `true` | 是否启用认证 |
| `KERT_AUTH_HEADER` | `X-API-Key` | 认证请求头名称 |
| `KERT_API_KEYS` | 空 | 逗号分隔的密钥声明 |

> 注意：`scripts/serve_skill_service.py` 的 `--host` 默认为 `0.0.0.0`（沿用既有行为），
> 该值会写入 `KERT_BIND_HOST` 参与校验。因此生产 profile 下若不启用认证，
...
`KERT_PROFILE=prod` 时，以下任一不满足即拒绝启动（抛 `ConfigError`）：
...
| 变量 | 默认 | 说明 |
|---|---|---|
| `KERT_RATE_LIMIT_ENABLED` | `false` | 是否启用限流 |
| `KERT_RATE_LIMIT_RPM` | `600` | 每分钟请求数 |
| `KERT_RATE_LIMIT_BURST` | `60` | 令牌桶容量（突发额度） |
| `KERT_SIZE_LIMIT_ENABLED` | `true` | 是否启用大小限制 |
| `KERT_MAX_REQUEST_BYTES` | `1048576` | 请求体上限 |
| `KERT_MAX_RESPONSE_BYTES` | `8388608` | 响应体上限 |
| `KERT_CONCURRENCY_ENABLED` | `false` | 是否启用并发上限 |
| `KERT_MAX_IN_FLIGHT` | `32` | 在途请求上限 |
| `KERT_CONCURRENCY_TIMEOUT` | `0` | 许可等待秒数，`0` 表示立即拒绝 |
...
| `KERT_RUNTIME_STORE_PATH` | `<ws>/90_control/runtime/runtime.db` | 数据库路径 |
| `KERT_RUNTIME_STORE_WAL` | `true` | 是否启用 WAL |
| `KERT_RUNTIME_STORE_BUSY_TIMEOUT_MS` | `5000` | 忙等超时 |
| `KERT_IDEMPOTENCY_TTL_SECONDS` | `600` | 幂等记录存活时长 |
...
export KERT_API_KEYS="gits-caller:<secret-at-least-16-chars>:read|execute"
export KERT_RATE_LIMIT_ENABLED=true
export KERT_CONCURRENCY_ENABLED=true
...
```bash
export KERT_CONFIG_FILE=./examples/config/runtime.prod.example.json
python scripts/serve_skill_service.py --workspace ./workspace
```
