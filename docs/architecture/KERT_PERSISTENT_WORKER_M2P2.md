# KERT 持久化异步 Worker 说明（M2-P2 / M2.4）
...
| 环境变量 | 默认 | 说明 |
|---|---|---|
| `KERT_WORKER_ID` | 自动生成 | 形如 `主机名-PID-随机后缀`（PID 可能复用，故加随机后缀） |
| `KERT_WORKER_JOB_TYPES` | 空（不限） | 逗号分隔的业务类型 |
| `KERT_WORKER_LEASE_SECONDS` | `30` | lease 租约时长 |
| `KERT_WORKER_POLL_INTERVAL` | `1.0` | 空闲轮询间隔 |
| `KERT_WORKER_RECLAIM_INTERVAL` | `5.0` | lease 回收扫描间隔 |
| `KERT_WORKER_MAX_JOBS` | `0`（不限） | 处理指定数量后退出 |
| `KERT_WORKER_BACKOFF_BASE` | `2.0` | 退避基数 |
| `KERT_WORKER_BACKOFF_FACTOR` | `2.0` | 退避因子 |
| `KERT_WORKER_BACKOFF_MAX` | `300.0` | 退避上限 |
...
profile 来源：构造参数 `profile=` 优先，缺省读 `KERT_PROFILE` 环境变量（默认 `dev`）。
