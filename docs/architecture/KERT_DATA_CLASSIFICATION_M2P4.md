...
| 环境变量 | 默认 | 说明 |
|---|---|---|
| `KERT_REDACT_RESPONSE` | **`false`** | 是否脱敏 JSON 响应 |
| `KERT_REDACT_THRESHOLD` | `RESTRICTED` | 脱敏阈值 |
| `KERT_REDACT_RESPONSE_TEXT` | `false` | 响应自由文本是否额外掩码 |
...
| 环境变量 | 默认 | 说明 |
|---|---|---|
| `KERT_LLM_REDACTION` | **`true`** | 是否在提示词出站前脱敏 |
...
export KERT_REDACT_RESPONSE=true
export KERT_REDACT_THRESHOLD=RESTRICTED
export KERT_LLM_REDACTION=true
...
export KERT_LLM_REDACTION=false
```
