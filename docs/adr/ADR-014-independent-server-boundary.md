...
KERT 被定位为独立服务端软件，但现有文档/部署/凭据/Skill 资产仍与外部智能体运行平台耦合。

## 决策

- KERT 产品运行不依赖任何 Agent Harness 或外部智能体运行平台
- 外部系统（GITS、智能体、前端）只能是 API 客户端
- 配置命名使用产品中性 `KERT_*`
- 密钥从 KERT 自身 Secret Provider 注入
- Skill 资产随 KERT 发布包自带，支持版本/哈希/签名

## 影响

- 需要 `kert-admin`、独立部署包、干净环境验收
