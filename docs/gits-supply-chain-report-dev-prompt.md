...
> 契约版本：KERT Skill 执行 API v1.2（`docs/dd/skill-execute-api-contract.md`）。
> 参考实现：KERT 服务端 `kert/src/kert/application/report.py` 与
...
## 背景
KERT（DeepSeek Harness 上的文件目录型知识服务模拟平台）已提供 Skill 执行服务：
...
- GET /api/skill/report/{requestId}：KERT 已提供定制「供应链图谱分析报告」HTML（Neo4j 风格力导向图谱 + 解读卡片 + 明细表）。但本需求要求 gits 侧自研展示，不依赖 KERT 报告页（KERT 页面仅作视觉参考）。
...
6. reportUrl 仅用于跳转参考；gits 侧数据一律取自 execute 响应 JSON，不解析 KERT 报告页 HTML。
...
### 为什么 gits 侧自研而不是直接用 KERT 报告页
- KERT 报告页服务于演示与联调；生产上 gits 需要把图谱嵌入自己的作战单/控制台，
...
- KERT 参考实现：`kert/src/kert/application/report.py`
- 视觉参考：`docs/dd/supply_chain_report_sample.html`（浏览器直接打开）
- 独立大图谱示例（1064 节点，Neo4j 风格，供样式参考）：`kert/examples/output/supply_chain_graph_1064.html`

### 服务端接口（gits 需要实现的）
| 方法 | 路径 | 说明 |
|---|---|---|
| POST | /api/skill/execute | 调用 KERT，取 data.result + data.reportUrl |
| GET | /api/skill/report/{requestId} | gits 自研报告页（数据取自本地保存的 execute 结果，勿回源 KERT） |
