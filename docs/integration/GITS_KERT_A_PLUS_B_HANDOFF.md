...
- GITS 工作区存在未提交改动：
  ...
  - `DshJobPoller.java`
  - `V14KertIntegrationController.java`
...
- 未配置 KERT、不可达、超时、鉴权失败、契约错误时，必须显示明确空态
- 禁止使用本地 Mock、H2、fallback 数据冒充 KERT 返回
- 禁止解析本地 HTML 报告页冒充图谱
- 禁止把 G0-G5 写成可写阶段机
...
- 验证超时、重试、鉴权失败、契约错误、KERT 不可用

## 3. 必须接入的 KERT 能力

| 能力 | Skill/端点 | 行为要求 |
|---|---|---|
| R1 访前报告 | `skill-customer-previsit-report` | 只展示 KERT `data.sections`；空则空态 |
...
```yaml
kert:
  base-url: "${KERT_BASE_URL:http://127.0.0.1:8106}"
  api-key: "${KERT_API_KEY:}"
...
注意：本候选建议 GITS 从 `dsh.*` 迁移到 `kert.*`，但具体迁移需 GITS 侧决策。
...
| 422 契约错误 | “KERT 契约错误” |
| 429 限流 | “KERT 繁忙，请稍后” |
| `status=skill_error` | “KERT 执行失败” |
| SP-20 `result.status=PARTIAL` | 展示规则违规，不允许对客放行 |
| 图谱空 | “KERT 未返回图谱” |

## 6. 验收清单

- [ ] GITS 不再使用 H2/Mock 拼装 KERT-owned 能力
- [ ] 未配置/不可达时页面显示空态
- [ ] 真实 HTTP 调用 KERT 成功
...
| HTTP 调用日志 | GITS/KERT | 提供 requestId 两侧日志 |
